...
def test_version_v422():
    assert Path("VERSION").read_text().strip()=="4.22.0-windows-agent-pull-now"
...
def test_agent_v11_command_loop_and_no_arbitrary_execution():
    ...
    assert 'AgentVersion = "1.1.0"' in src
...
def test_agent_upgrade_preserves_existing_enrollment():
    ...
    assert 'Existing enrollment and DPAPI-protected API key preserved.' in ps
