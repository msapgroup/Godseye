...
def test_v34_version():
    assert Path("VERSION").read_text().strip() == "3.4.0-device-cleanup-expanded-alert-rules"
...
def test_inventory_has_cleanup_and_per_device_delete_ui():
    ...
    assert "Keep Known and Managed devices" in html
...
def test_bulk_cleanup_deletes_old_unprotected_but_keeps_online_and_protected(tmp_path):
    from app import main
    from fastapi.testclient import TestClient
    old_db = main.DB_PATH
    main.DB_PATH = tmp_path / "v34.db"
    try:
        main.init_db()
        ...
        with main.db() as c:
            rows = [
                ...
                ("AA:00:00:00:00:03", "10.0.0.3", "Online old timestamp", "online", old, "new"),
        ...
        with TestClient(main.app) as client:
            ...
            r = client.post("/api/v1/devices/cleanup", headers=headers, json={"mode":"old","older_than_days":30,"exclude_protected":True})
            assert r.status_code == 200, r.text
            assert r.json()["deleted"] == 1
        with main.db() as c:
            remaining = {r["mac"] for r in c.execute("SELECT mac FROM devices")}
            assert "AA:00:00:00:00:01" not in remaining
            assert "AA:00:00:00:00:02" in remaining
            assert "AA:00:00:00:00:03" in remaining
            assert "AA:00:00:00:00:04" in remaining
            audit = c.execute("SELECT details FROM audit_log WHERE action='device_cleanup' ORDER BY id DESC LIMIT 1").fetchone()
            assert audit and "deleted=1" in audit["details"]
    finally:
        main.DB_PATH = old_db
...
def test_individual_delete_cleans_owned_history_and_preserves_audit(tmp_path):
    ...
    try:
        ...
        with TestClient(main.app) as client:
            headers = _login_admin(main, client)
            r = client.delete(f"/api/v1/devices/{did}", headers=headers)
            assert r.status_code == 200, r.text
        with main.db() as c:
            ...
            assert c.execute("SELECT COUNT(*) FROM audit_log WHERE action='device_deleted' AND target='AA:BB:CC:DD:EE:01'").fetchone()[0] == 1
    finally:
        main.DB_PATH = old_db
