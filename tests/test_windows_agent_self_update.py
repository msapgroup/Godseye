...
def test_manifest_loader_rejects_bad_filename_and_hash(tmp_path):
    p=tmp_path/"manifest.json";p.write_text('{"version":"2.2.2","filename":"evil.exe","sha256":"A"}')
...
def test_upgrade_queues_only_version_and_hash(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(windows_agent,"load_update_manifest",lambda path:manifest("2.2.2"))
    ...
    with main.db() as c:
        ...
        assert payload=={"version":"2.2.2","sha256":"A"*64}
...
def test_x64_agent_updater_is_fixed_hash_verified_msi_path():
    ...
    assert '<Version>2.2.2</Version>' in csproj
