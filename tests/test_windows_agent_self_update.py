...
def test_manifest_loader_rejects_bad_filename_and_hash(tmp_path):
    p=tmp_path/"manifest.json";p.write_text('{"version":"2.2.4","filename":"evil.exe","sha256":"A"}')
...
def test_upgrade_queues_only_version_and_hash(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(windows_agent,"load_update_manifest",lambda path:manifest("2.2.4"))
    ...
    with main.db() as c:
        ...
        assert payload=={"version":"2.2.4","sha256":"A"*64}
