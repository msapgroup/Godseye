...
def test_v230_source_and_installer_contracts():
    ...
    assert '<FileVersion>2.4.4.0</FileVersion>' in csproj
    assert '#define MyAppVersion "2.4.4"' in iss
    assert '#define MyMsiName "GODSEYE-Windows-Agent-x64.msi"' in iss
    assert 'Version="2.4.4"' in wix
    assert 'Local\\GODSEYE.WindowsAgent.Tray.2.4.4' in tray
...
def test_pending_manifest_cannot_advertise_stale_agent(tmp_path):
    ...
    manifest.write_text(json.dumps({"status":"pending_build","version":"2.4.4","filename":"GODSEYE-Windows-Agent-x64.msi","sha256":""}),encoding="utf-8")
