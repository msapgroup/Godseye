...
- `docs/screenshots/` — current dark-mode UI documentation images
...
## v3.1.0 — Integrations, Analytics & Notifications

GODSEYE now supports multiple Pi-hole, UniFi, and SNMP collectors at the same time. Integration settings are managed as named cards, Client & Query Analytics open into focused per-integration dialogs with reason-required clearing, and notification settings are grouped by delivery channel for a clearer workflow.
