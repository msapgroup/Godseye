...
The installer compiles and installs the 64-bit `GODSEYEWindowsAgent` Windows service using the built-in .NET Framework C# compiler. The agent makes outbound HTTPS requests only; no inbound WinRM listener is required. Agent v1.1 keeps a lightweight 10-second management heartbeat for **Pull Events Now** and targeted rechecks while preserving the separately configured normal collection interval. Its long-term machine API key is protected with Windows DPAPI LocalMachine and the local data directory is ACL-restricted to SYSTEM and Administrators. Existing enrolled v1.0 agents can be upgraded with the v1.1 package without a new enrollment token. See `docs/WINDOWS_AGENT.md`.
...
## v4.25 Windows Agent baseline

GODSEYE v4.25 packages the independently-versioned **Windows Agent 2.2.7** installer for Windows Event Findings and user-approved Remote Access. Normal GODSEYE server upgrades do not require reinstalling an already-compatible 2.x agent. The Windows Agent uses its stable `/api/v1/windows-agents/...` API contract and can receive hash-verified MSI updates through GODSEYE when a newer compatible agent is published.
