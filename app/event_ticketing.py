...
def ingest_event(c, source_id: int | None, event: dict, now_iso: str, agent_id: int | None = None) -> tuple[int,bool]:
    ...
    if existing:
        ...
        c.execute("""UPDATE event_findings SET source_id=COALESCE(?,source_id),agent_id=COALESCE(?,agent_id),level=?,category=?,severity=?,title=?,message=?,record_id=?,event_time=?,
                     ...
                     WHERE id=?""",
                  (source_id,agent_id,level,rule["category"],rule["severity"],rule["title"],message,record_id,event_time,rule["recommendation"],json.dumps(rule["actions"]),
                   new_status,now_iso,new_status,existing["id"]))
        finding_id=existing["id"]
    else:
        cur=c.execute("""INSERT INTO event_findings(source_id,agent_id,computer_name,channel,provider,event_id,level,category,severity,title,message,record_id,event_time,
                         finding_key,recommendation,suggested_actions_json,status,occurrence_count,first_seen,last_seen)
                         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'open',1,?,?)""",
                      (source_id,agent_id,computer,channel,provider,event_id,level,rule["category"],rule["severity"],rule["title"],message,record_id,event_time,key,
