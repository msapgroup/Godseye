...
def config_export(c):
    ...
    integrations=[]
    for r in rows('SELECT name,kind,enabled,target,username,verify_tls,sync_interval_seconds,options_json FROM integration_configs ORDER BY kind,name'):
...
def config_import(c, payload):
    ...
    for r in payload.get('integrations',[]):
        ...
        ts=utcnow(); name=(r.get('name') or (r['kind'].title()+' 1')).strip()
        existing=c.execute('SELECT id,secret FROM integration_configs WHERE kind=? AND name=?',(r['kind'],name)).fetchone()
        if existing:
            c.execute('''UPDATE integration_configs SET enabled=?,target=?,username=?,verify_tls=?,sync_interval_seconds=?,options_json=?,updated_at=? WHERE id=?''',(int(bool(r.get('enabled'))),r.get('target',''),r.get('username',''),int(bool(r.get('verify_tls',True))),max(30,int(r.get('sync_interval_seconds',300))),json.dumps(r.get('options',{})),ts,existing['id']))
        else:
            c.execute('''INSERT INTO integration_configs(name,kind,enabled,target,username,secret,verify_tls,sync_interval_seconds,options_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)''',(name,r['kind'],int(bool(r.get('enabled'))),r.get('target',''),r.get('username',''),'',int(bool(r.get('verify_tls',True))),max(30,int(r.get('sync_interval_seconds',300))),json.dumps(r.get('options',{})),ts,ts))
    ...
    traffic=payload.get('traffic_collection') or {}
    if traffic:
        try:
            integration_id=None
            if traffic.get('integration_name') and traffic.get('integration_kind'):
                row=c.execute('SELECT id FROM integration_configs WHERE kind=? AND name=?',
                              (traffic.get('integration_kind'),traffic.get('integration_name'))).fetchone()
                integration_id=row['id'] if row else None
            c.execute('''UPDATE traffic_collection_settings
                         SET enabled=?,mode=?,integration_id=?,interface=?,sample_interval_seconds=?,options_json=?,updated_at=?
                         WHERE id=1''',
                      (int(bool(traffic.get('enabled'))),traffic.get('mode','unifi'),integration_id,
                       traffic.get('interface',''),max(10,int(traffic.get('sample_interval_seconds',30))),
                       traffic.get('options_json','{}') if isinstance(traffic.get('options_json','{}'),str)
                       else json.dumps(traffic.get('options_json') or {}),utcnow()))
        except sqlite3.OperationalError:
            pass
