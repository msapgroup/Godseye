...
class CollectorManager:
    ...
    def _run(self, setting):
        ...
        with self.db_factory() as c:
            ensure_schema(c)
            c.execute(
                "INSERT INTO integration_checks(kind,target,status,latency_ms,details,last_checked,monitor_id) VALUES(?,?,?,?,?,?,?)",
                (kind,target,status,round(latency,1),json.dumps(result)[:10000],self.now(),setting.get("id"))
            )
            ...
            from .remediation import monitor_result_findings
            monitor_result_findings(c,setting,result,status,round(latency,1))
    ...
    def run_one(self,monitor_id):
        with self.db_factory() as c: row=c.execute("SELECT * FROM integration_settings WHERE id=?",(monitor_id,)).fetchone()
