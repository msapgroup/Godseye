...
FULL_DISCOVERY_INTERVAL = int(os.environ.get("GODSEYE_FULL_DISCOVERY_INTERVAL", "300"))
_last_full_discovery = 0.0
    ...
    with db() as c:
        ...
        for d in found:
            ...
            if old is None:
                ...
                correlate_device(c, mac, d["ip"], hostname, d["vendor"], source="arp-scan", details=d)
                ...
                correlate_device(c, mac, d["ip"], old["hostname"], d["vendor"] or old["vendor"], source="arp-scan", details=d)
...
def evaluate_rules():
    ...
    Supported rule families cover discovery bursts, prolonged offline devices,
    IP changes, reconnect storms, total offline-device count, scanner health,
    and classification counts. Burst/count rules use a cooldown so a persistent
    condition does not generate an alert every scanner cycle.
    """
    timestamp = now()
    fired_events = []
    with db() as c:
        ...
        for rule in rules:
            ...
            if rule["rule_type"] == "new_device_burst":
                ...
                if candidates:
                    c.execute("UPDATE rules SET last_triggered_at=? WHERE id=?", (timestamp, rule["id"]))
            elif rule["rule_type"] in {"ip_change_burst", "reconnect_burst"}:
                count_threshold = int(params.get("count", 3))
                window_minutes = float(params.get("window_minutes", 10))
                if rule["last_triggered_at"]:
                    since_last = (dt.datetime.now(dt.timezone.utc) - dt.datetime.fromisoformat(rule["last_triggered_at"])).total_seconds() / 60
                    if since_last < window_minutes:
                        continue
                event_type = "ip_changed" if rule["rule_type"] == "ip_change_burst" else "connected"
                window_start = (dt.datetime.now(dt.timezone.utc) - dt.timedelta(minutes=window_minutes)).isoformat()
                actual_count = c.execute(
                    "SELECT COUNT(*) FROM events WHERE event_type=? AND created_at >= ?", (event_type, window_start)
                ).fetchone()[0]
                if actual_count >= count_threshold:
                    label = "IP changes" if event_type == "ip_changed" else "device reconnects"
                    fired_events.append(_log_event(
                        c, None, "rule_triggered", None, timestamp,
                        f"Rule '{rule['name']}': {actual_count} {label} in the last {window_minutes:g} minute(s) "
                        f"(threshold {count_threshold})",
                        severity=rule["severity"],
                    ))
                    c.execute("UPDATE rules SET last_triggered_at=? WHERE id=?", (timestamp, rule["id"]))

            elif rule["rule_type"] == "offline_count":
                count_threshold = int(params.get("count", 5))
                cooldown_minutes = float(params.get("cooldown_minutes", 15))
                if rule["last_triggered_at"]:
                    since_last = (dt.datetime.now(dt.timezone.utc) - dt.datetime.fromisoformat(rule["last_triggered_at"])).total_seconds() / 60
                    if since_last < cooldown_minutes:
                        continue
                actual_count = c.execute("SELECT COUNT(*) FROM devices WHERE status='offline'").fetchone()[0]
                if actual_count >= count_threshold:
                    fired_events.append(_log_event(
                        c, None, "rule_triggered", None, timestamp,
                        f"Rule '{rule['name']}': {actual_count} devices are offline (threshold {count_threshold})",
                        severity=rule["severity"],
                    ))
                    c.execute("UPDATE rules SET last_triggered_at=? WHERE id=?", (timestamp, rule["id"]))

            elif rule["rule_type"] == "scanner_stale":
                minutes_threshold = float(params.get("minutes", 10))
                hb = c.execute("SELECT last_success_at FROM scanner_heartbeat WHERE id=1").fetchone()
                last_success = hb["last_success_at"] if hb else None
                stale = not last_success
                age_minutes = None
                if last_success:
                    age_minutes = (dt.datetime.now(dt.timezone.utc) - dt.datetime.fromisoformat(last_success)).total_seconds() / 60
                    stale = age_minutes >= minutes_threshold
                if stale:
                    if rule["last_triggered_at"]:
                        since_last = (dt.datetime.now(dt.timezone.utc) - dt.datetime.fromisoformat(rule["last_triggered_at"])).total_seconds() / 60
                        if since_last < minutes_threshold:
                            continue
                    detail = "scanner has never completed successfully" if age_minutes is None else f"last successful scan was {age_minutes:.1f} minutes ago"
                    fired_events.append(_log_event(
                        c, None, "rule_triggered", None, timestamp,
                        f"Rule '{rule['name']}': {detail} (threshold {minutes_threshold:g} minutes)",
                        severity=rule["severity"],
                    ))
                    c.execute("UPDATE rules SET last_triggered_at=? WHERE id=?", (timestamp, rule["id"]))

            elif rule["rule_type"] == "classification_count":
                count_threshold = int(params.get("count", 1))
                cooldown_minutes = float(params.get("cooldown_minutes", 15))
                classifications = params.get("classifications") or ["new", "investigate"]
                if rule["last_triggered_at"]:
                    since_last = (dt.datetime.now(dt.timezone.utc) - dt.datetime.fromisoformat(rule["last_triggered_at"])).total_seconds() / 60
                    if since_last < cooldown_minutes:
                        continue
                placeholders = ",".join("?" * len(classifications))
                actual_count = c.execute(
                    f"SELECT COUNT(*) FROM devices WHERE classification IN ({placeholders})", classifications
                ).fetchone()[0]
                if actual_count >= count_threshold:
                    fired_events.append(_log_event(
                        c, None, "rule_triggered", None, timestamp,
                        f"Rule '{rule['name']}': {actual_count} devices are classified as {', '.join(classifications)} "
                        f"(threshold {count_threshold})",
                        severity=rule["severity"],
                    ))
                    c.execute("UPDATE rules SET last_triggered_at=? WHERE id=?", (timestamp, rule["id"]))
...
def claim_scan_request():
    """Atomically claim the oldest pending manual scan request."""
    with db() as c:
        row = c.execute("SELECT * FROM scan_requests WHERE status='pending' ORDER BY id LIMIT 1").fetchone()
        if not row:
            return None
        started = now()
        c.execute("UPDATE scan_requests SET status='running', started_at=? WHERE id=? AND status='pending'", (started, row["id"]))
        if c.execute("SELECT changes()").fetchone()[0] != 1:
            return None
        return row["id"]

def finish_scan_request(request_id, status, devices_found=None, error=None):
    with db() as c:
        c.execute("UPDATE scan_requests SET status=?,completed_at=?,devices_found=?,error=? WHERE id=?",
                  (status, now(), devices_found, error, request_id))

def run_one_scan(request_id=None):
    ...
    try:
        ...
        if found is None:
            ...
            if request_id is not None:
                finish_scan_request(request_id, "failed", error="scanner could not determine subnet or arp-scan failed")
            return []
        fired = record_scan(found)
        # Run the richer discovery pipeline on every manual scan and periodically
        # during scheduled scans. Failure of an optional source never fails ARP.
        global _last_full_discovery
        force_full = request_id is not None
        if force_full or (time.monotonic() - _last_full_discovery) >= FULL_DISCOVERY_INTERVAL:
            try:
                from .discovery_intelligence import run_full_discovery
                subnet = local_subnet()
                with db() as c:
                    summary = run_full_discovery(c, str(subnet) if subnet else None, arp_observations=found)
                _last_full_discovery = time.monotonic()
                print(f"[GODSEYE] full discovery: {summary.get('total_observations',0)} correlated observations")
            except Exception as exc:
                print(f"[GODSEYE] full discovery warning: {exc}")
        record_heartbeat(success=True, devices_found=len(found), duration_ms=duration_ms)
        if request_id is not None:
            finish_scan_request(request_id, "completed", devices_found=len(found))
        return fired
    except Exception as exc:
        record_heartbeat(success=False, duration_ms=int((time.monotonic() - start) * 1000), error=str(exc))
        if request_id is not None:
            finish_scan_request(request_id, "failed", error=str(exc))
...
def main():
    ...
    # Perform one normal scan immediately on service startup.
    fired_events = run_one_scan()
    fired_events += evaluate_rules()
    try:
        from .remediation import scanner_event_findings
        with db() as c: scanner_event_findings(c,fired_events)
    except Exception as exc:
        print(f"[GODSEYE] finding evaluation warning: {exc}")
    dispatch_events(fired_events)
    while True:
        request_id = claim_scan_request()
        if request_id is not None:
            fired_events = run_one_scan(request_id)
            fired_events += evaluate_rules()
            try:
                from .remediation import scanner_event_findings
                with db() as c: scanner_event_findings(c,fired_events)
            ...
            dispatch_events(fired_events)
            continue
        # Poll the request queue every second so a manual Scan Now starts
        # promptly rather than waiting for the normal interval.
        for _ in range(SCAN_INTERVAL):
            with db() as c:
                pending = c.execute("SELECT 1 FROM scan_requests WHERE status='pending' LIMIT 1").fetchone()
            if pending:
                break
            time.sleep(1)
        else:
