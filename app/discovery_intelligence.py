...
def pihole_client_observations(base_url, token=None, timeout=8):
    """Best-effort Pi-hole v6 network/client discovery.

    Supports /api/network/devices and /api/clients style JSON. An optional
    token is sent as X-FTL-SID and Bearer authorization for compatibility with
    common local deployments.
    """
    if not base_url:
        return {"ok": False, "observations": [], "error": "Pi-hole URL not configured"}
    headers = {"User-Agent": "GODSEYE/1.3"}
    if token:
        headers["X-FTL-SID"] = token
        headers["Authorization"] = "Bearer " + token
    errors = []
    for path in ("/api/network/devices", "/api/clients"):
        try:
            ...
            for row in rows or []:
                ...
                ip = row.get("ip") or row.get("ip_address") or row.get("lastQuery")
...
def run_full_discovery(c, network=None, integration_settings=None, arp_observations=None):
    ...
    for setting in settings:
        ...
        if kind == "pihole":
            result = pihole_client_observations(setting.get("target"), opts.get("token") or opts.get("api_token"))
            add("pihole", result)
        elif kind == "unifi":
