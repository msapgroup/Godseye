...
def delete_device(device_id: int, request: Request, admin=Depends(require_admin)):
    with db() as c:
        ...
        audit(c, admin["username"], "device_deleted", target=row["mac"],
              details=f"name={row.get('name') or row.get('hostname') or ''}; status={row.get('status')}; last_seen={row.get('last_seen')}",
              ip=client_ip(request))
...
def cleanup_devices(payload: DeviceCleanupRequest, request: Request, admin=Depends(require_admin)):
    ...
    with db() as c:
        if payload.mode == "offline":
            ...
            # Old cleanup is intentionally limited to devices that are not currently online.
            where = "status!='online' AND last_seen < ?"
            values = [cutoff]
        if payload.exclude_protected:
            where += " AND classification NOT IN ('known','managed')"
        ...
        audit(c, admin["username"], "device_cleanup", target=payload.mode,
              details=f"deleted={deleted}; older_than_days={payload.older_than_days}; exclude_protected={payload.exclude_protected}",
              ip=client_ip(request))
...
<div id="deviceCleanupModal" class="modal" style="display:none" onclick="if(event.target===this)closeDeviceCleanup()">
  <div class="modal-card">
    ...
    <form onsubmit="return submitDeviceCleanup(event)" class="modal-form">
      ...
      <label id="deviceCleanupDaysLabel" style="display:none">Older than<input class="input" id="deviceCleanupDays" type="number" min="1" max="3650" value="30"><span class="muted">days since last seen. Currently-online devices are never deleted by old-device cleanup.</span></label>
      <label style="display:flex;align-items:center;gap:10px"><input id="deviceCleanupProtected" type="checkbox" checked> Keep Known and Managed devices</label>
      <div class="muted">This removes matching devices and their device-owned event/source/IP history. If a deleted device returns later, discovery can add it again.</div>
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.getElementById('deviceCleanupModal')?.style.display==='grid')closeDeviceCleanup()});
function openDeviceCleanup(){const m=document.getElementById('deviceCleanupModal');if(!m)return;m.style.display='grid';document.body.style.overflow='hidden';updateDeviceCleanupFields()}
...
async function submitDeviceCleanup(e){e.preventDefault();const mode=document.getElementById('deviceCleanupMode').value,older_than_days=+document.getElementById('deviceCleanupDays').value,exclude_protected=document.getElementById('deviceCleanupProtected').checked,err=document.getElementById('deviceCleanupErr');if(err)err.textContent='';const label=mode==='offline'?'all matching offline devices':`matching devices not seen for ${older_than_days}+ days`;if(!confirm(`Delete ${label}? This cannot be undone.`))return false;try{const r=await json('/api/v1/devices/cleanup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode,older_than_days,exclude_protected})});closeDeviceCleanup();await Promise.allSettled([loadInventory(),loadDashboard(),loadNetwork()]);alert(`Deleted ${r.deleted||0} device(s).`)}catch(ex){if(err)err.textContent='Cleanup failed: '+ex.message}return false}
async function deleteInventoryDevice(id,name,status){if(!confirm(`Delete ${name||'this device'} from inventory?${status==='online'?'\n\nWarning: this device is currently online.':''}`))return;try{await json('/api/v1/devices/'+id,{method:'DELETE'});await Promise.allSettled([loadInventory(),loadDashboard(),loadNetwork()])}catch(e){alert('Could not delete device: '+e.message)}}
