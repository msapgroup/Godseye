...
app = FastAPI(title="GODSEYE", version="2.3.0", lifespan=lifespan)
...
def update_device(device_id: int, payload: DeviceUpdate, request: Request, admin=Depends(require_admin)):
    fields, values = [], []
    for field in ("name", "hostname", "classification", "notes", "device_type"):
