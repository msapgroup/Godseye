...
/* v4.24 — Windows Agent Remote Access console */
...
<div class="remote-session-head"><div><h2 id="remoteSessionTitle">Remote Session</h2><div class="muted" id="remoteSessionStatus">Select an online computer to begin.</div></div><div class="actions"><span class="remote-ready"><span class="remote-dot online"></span>Ready</span><button id="remoteScreenshotBtn" class="secondary" type="button" onclick="openRemoteScreenshot()" disabled>Take Screenshot</button><button id="remoteDisconnectBtn" class="danger" type="button" onclick="stopRemoteSession()" disabled>Disconnect</button></div></div>
...
function remoteAgo(v){if(!v)return'—';const t=new Date(v).getTime();if(!Number.isFinite(t))return'—';const s=Math.max(0,Math.floor((Date.now()-t)/1000));if(s<60)return s+' sec ago';if(s<3600)return Math.floor(s/60)+' min ago';if(s<86400)return Math.floor(s/3600)+' hr ago';return Math.floor(s/86400)+' day'+(s>=172800?'s':'')+' ago'}
