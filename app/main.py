...
async function deleteIntegration(id){const c=INTEGRATIONS.find(x=>x.id===id);if(!confirm('Delete '+(c?.name||'this integration')+'? Saved analytics will remain until cleared.'))return;try{await json('/api/v1/integrations/configs/'+id,{method:'DELETE'});await loadIntegrations()}catch(e){alert('Could not delete integration: '+e.message)}}
...
def delete_integration(integration_id: int, request: Request, user=Depends(require_admin)):
    with db() as c:
        ...
        audit(c,user['username'],'integration_config_deleted',str(integration_id),f"{row['kind']} {row['name']}",client_ip(request))
    return {"ok":True}
