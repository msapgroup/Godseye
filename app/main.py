...
function renderCalendar(){
 ...
 for(let i=0;i<42;i++){
  ...
  html+=`<div class="calendar-day ${sameMonth?'':'outside'} ${isToday?'today':''}" ondblclick="openCalendarEventModal(null,'${calendarIsoLocal(new Date(day.getFullYear(),day.getMonth(),day.getDate(),9,0))}')"><div class="calendar-day-num">${day.getDate()}</div>`;
  events.slice(0,4).forEach(e=>{
    ...
    html+=`<button class="calendar-event-chip ${esc(e.color||'blue')}${synced}${readonly}" title="${esc(e.title)}" onclick="openCalendarEventModal(${e.id})">${integration?'<span class="calendar-sync-mark">↻</span> ':''}${esc(start+e.title)}</button>`;
  });
...
function openCalendarEventModal(id=null,startValue=null){
 ...
 calendarDeleteBtn.style.display=event?'inline-flex':'none';
...
function closeCalendarEventModal(){calendarEventModal.style.display='none';document.body.style.overflow='';calendarEventTarget.disabled=false}
