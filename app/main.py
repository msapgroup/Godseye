...
function renderCalendar(){
 ...
 for(let i=0;i<42;i++){
  ...
  events.slice(0,4).forEach(e=>{
    ...
    html+=`<button class="calendar-event-chip ${esc(e.color||'blue')}${synced}${readonly}" title="${esc(e.title)}" onclick="event.stopPropagation();openCalendarEventModal(${e.id})" ondblclick="event.stopPropagation();openCalendarEventModal(${e.id})">${integration?'<span class="calendar-sync-mark">↻</span> ':''}${esc(start+e.title)}</button>`;
  });
