...
app = FastAPI(title="GODSEYE", version="1.8.0", lifespan=lifespan)
...
<div class="card statcard"><div class="staticon purple">◔</div><div><div class="statmeta">Monitors</div><div class="statnum" id="monitorCount">—</div></div><span class="trend" id="monitorTrend">—</span></div>
</div>
...
<div class="view" id="view-device" style="display:none">
  <div class="hero">
    <div class="device-page-head"><button class="back-btn" type="button" onclick="returnToDevices()">← Back to Devices</button><div><h1 id="devicePageTitle">Device</h1><div class="muted" id="devicePageSubtitle">Device intelligence and activity</div></div></div>
    <div class="actions"><button class="secondary" onclick="recheckActiveDevice()">↻ Recheck</button><button class="primary" onclick="diagnoseActiveDevice()">Diagnose</button></div>
  </div>
  <div id="deviceDiagOut"></div><div id="devicePageContent"><div class="empty">Select a device from the inventory.</div></div>
</div>
...
const VIEW_LOADERS={
  ...
  activity:()=>load(),
  device:()=>ACTIVE_DEVICE_ID?openDevicePage(ACTIVE_DEVICE_ID,false):null
};
...
function installSidebarNavigation(){
  ...
  window.addEventListener('hashchange',()=>{
    ...
    const dm=name.match(/^device-(\d+)$/);
    if(dm){openDevicePage(Number(dm[1]),false);return}
...
async function loadDevices(){
  try{
    let q=new URLSearchParams();if(document.getElementById('search')?.value)q.set('search',search.value);if(document.getElementById('status')?.value)q.set('status',status.value);if(document.getElementById('classification')?.value)q.set('classification',classification.value);
    let d=await json('/api/v1/devices?'+q);
    const rows=d.slice(0,12);
    devices.innerHTML=rows.length?rows.map(x=>`<tr onclick="openDevicePage(${x.id})"><td class="${esc(x.status)}"><span class="dot">●</span> ${esc((x.status||'unknown').replace('_',' '))}</td><td><button class="device-link" onclick="event.stopPropagation();openDevicePage(${x.id})">${esc(x.name||x.hostname||'Unknown device')}</button><div class="muted">${esc(x.device_type||'Unclassified')}</div></td><td>${esc(x.ip||'—')}</td><td>${esc(x.mac||'—')}</td><td>${esc(x.vendor||'—')}</td><td>${esc(x.device_type||'—')}</td><td>${x.last_seen?esc(new Date(x.last_seen).toLocaleString()):'—'}</td></tr>`).join(''):'<tr><td colspan="7" class="empty">No devices discovered yet. Click Scan Now to discover the network.</td></tr>';
  }catch(e){devices.innerHTML='<tr><td colspan="7" class="empty">Unable to load devices: '+esc(e.message)+'</td></tr>'}
}
...
async function loadInventory(){try{let q=new URLSearchParams();if(inventorySearch.value)q.set('search',inventorySearch.value);if(inventoryStatus.value)q.set('status',inventoryStatus.value);let d=await json('/api/v1/devices?'+q);inventoryCount.textContent=d.length+' devices';inventoryRows.innerHTML=d.length?d.map(x=>`<tr onclick="openDevicePage(${x.id})" style="cursor:pointer"><td><div class="device-name"><span class="device-icon">●</span><div><button class="link name" onclick="event.stopPropagation();openDevicePage(${x.id})">${esc(x.name||x.hostname||'Unknown device')}</button><div class="muted">${esc(x.device_type||'Unclassified')}</div></div></div></td><td>${esc(x.ip||'—')}</td><td>${esc(x.mac)}</td><td>${esc(x.vendor||'—')}</td><td>${esc(x.device_type||'—')}</td><td>${x.last_seen?esc(new Date(x.last_seen).toLocaleString()):'—'}</td><td><span class="status-badge">${esc(x.status||'unknown')}</span></td><td><button class="secondary" onclick="event.stopPropagation();openDevicePage(${x.id})">Details</button></td></tr>`).join(''):'<tr><td colspan="8" class="empty">No devices discovered yet. Click Scan Now to discover the network.</td></tr>'}catch(e){inventoryRows.innerHTML='<tr><td colspan="8" class="empty">Unable to load devices.</td></tr>'}}
let ACTIVE_DEVICE_ID=null;
let LAST_DEVICE_VIEW='devices';
function closeDeviceIntel(){returnToDevices()}
function returnToDevices(){ACTIVE_DEVICE_ID=null;showView(LAST_DEVICE_VIEW||'devices')}
async function openDevicePage(id,updateHash=true){
  ACTIVE_DEVICE_ID=id;LAST_DEVICE_VIEW='devices';showView('device',false);
  const box=document.getElementById('devicePageContent');
  box.innerHTML='<div class="empty">Loading device intelligence…</div>';
  try{const r=await json('/api/v1/devices/'+id+'/intelligence');renderDeviceIntel(r);if(updateHash)history.replaceState(null,'','#device-'+id)}
  catch(e){box.innerHTML='<div class="err">Unable to load device intelligence: '+esc(e.message)+'</div>'}
}
function fmtAgo(sec){if(sec===null||sec===undefined)return '—';sec=Number(sec);if(sec<60)return Math.round(sec)+' sec ago';if(sec<3600)return Math.round(sec/60)+' min ago';if(sec<86400)return Math.round(sec/3600)+' hr ago';return Math.round(sec/86400)+' d ago'}
function renderDeviceIntel(r){
  const d=r.device||{},s=r.summary||{},risk=Number(s.risk_score||0),events=(r.events||[]).slice(0,20),ips=(r.ip_history||[]).slice(0,12),issues=(r.issues||[]).filter(x=>x.status==='open'),sources=r.sources||[];
  devicePageTitle.textContent=d.name||d.hostname||'Unknown device';devicePageSubtitle.textContent=[d.ip||'No IP',d.mac||'No MAC',d.vendor||'Unknown vendor'].join(' · ');
  devicePageContent.innerHTML=`<div class="device-detail-shell"><div class="stack"><section class="panel device-hero-card"><div style="display:flex;justify-content:space-between;gap:16px;align-items:start"><div><div class="muted">${esc(d.device_type||'Unclassified')}</div><h2 style="padding:0;border:0;margin:4px 0 0">${esc(d.name||d.hostname||'Unknown device')}</h2></div><span class="status-badge">${esc(d.status||'unknown')}</span></div><div class="kpi-row"><div class="kpi"><div class="k">Risk score</div><div class="v">${risk}/100</div></div><div class="kpi"><div class="k">Evidence sources</div><div class="v">${s.source_count||0}</div></div><div class="kpi"><div class="k">Known IPs</div><div class="v">${s.known_ip_count||0}</div></div><div class="kpi"><div class="k">Last seen</div><div class="v">${fmtAgo(s.seconds_since_seen)}</div></div></div></section><section class="panel"><div class="table-head"><h2>Recent Events</h2><div class="muted">Latest device activity</div></div><div style="overflow:auto"><table><thead><tr><th>Time</th><th>Event</th><th>IP</th><th>Details</th></tr></thead><tbody>${events.length?events.map(x=>`<tr><td>${x.created_at?esc(new Date(x.created_at).toLocaleString()):'—'}</td><td>${esc(x.event_type||'—')}</td><td>${esc(x.ip||'—')}</td><td>${esc(x.details||'')}</td></tr>`).join(''):'<tr><td colspan="4" class="empty">No events recorded.</td></tr>'}</tbody></table></div></section><section class="panel"><div class="table-head"><h2>IP History</h2></div><div style="overflow:auto"><table><thead><tr><th>IP</th><th>Source</th><th>First Seen</th><th>Last Seen</th></tr></thead><tbody>${ips.length?ips.map(x=>`<tr><td>${esc(x.ip)}</td><td>${esc(x.source)}</td><td>${x.first_seen?esc(new Date(x.first_seen).toLocaleString()):'—'}</td><td>${x.last_seen?esc(new Date(x.last_seen).toLocaleString()):'—'}</td></tr>`).join(''):'<tr><td colspan="4" class="empty">No IP history recorded.</td></tr>'}</tbody></table></div></section></div><div class="stack"><section class="panel"><div class="table-head"><h2>Identity</h2></div><div style="padding:16px" class="intel-kv"><div class="k">Hostname</div><div>${esc(d.hostname||'—')}</div><div class="k">IP address</div><div>${esc(d.ip||'—')}</div><div class="k">MAC address</div><div>${esc(d.mac||'—')}</div><div class="k">Vendor</div><div>${esc(d.vendor||'—')}</div><div class="k">Classification</div><div>${esc(d.classification||'—')}</div><div class="k">First seen</div><div>${d.first_seen?esc(new Date(d.first_seen).toLocaleString()):'—'}</div></div></section><section class="panel"><div class="table-head"><h2>Evidence Sources</h2></div><div style="padding:16px">${sources.length?sources.map(x=>`<div class="timeline-row"><b>${esc(x.source)}</b><div class="muted">${esc(x.ip||'—')} · ${x.last_seen?esc(new Date(x.last_seen).toLocaleString()):'—'}</div></div>`).join(''):'<div class="empty">Inventory evidence only.</div>'}</div></section><section class="panel"><div class="table-head"><h2>Open Findings</h2></div><div style="padding:16px">${issues.length?issues.map(x=>`<div class="timeline-row"><b>${esc(x.title)}</b><div class="muted">${esc(x.recommendation||'')}</div></div>`).join(''):'<div class="empty">No open findings.</div>'}</div></section><section class="panel"><div class="table-head"><h2>Recommendations</h2></div><div style="padding:16px"><ul class="intel-list">${(r.recommendations||[]).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>No recommendations right now.</li>'}</ul></div></section></div></div>`;
}
...
async function recheckActiveDevice(){if(!ACTIVE_DEVICE_ID)return;const out=document.getElementById('deviceDiagOut');out.innerHTML='<div class="panel" style="padding:12px">Rechecking device…</div>';try{const r=await json('/api/v1/devices/'+ACTIVE_DEVICE_ID+'/recheck',{method:'POST'});out.innerHTML=`<div class="panel" style="padding:12px"><b>${r.ok?'✓ Device is reachable':'⚠ Device is still unreachable'}</b></div>`;await openDevicePage(ACTIVE_DEVICE_ID,false);await loadInventory();await load()}catch(e){out.innerHTML='<div class="err">Recheck failed: '+esc(e.message)+'</div>'}}
...
async function loadTraffic(){
  ...
  try{
    ...
    const c=r.clients||{};note.textContent=c.note||'Observed activity by device';const rows=c.clients||[],mx=Math.max(1,...rows.map(x=>Number(x.activity_events)||0));client.innerHTML=rows.length?rows.slice(0,10).map(x=>`<div style="display:grid;grid-template-columns:minmax(130px,220px) 1fr 60px;gap:10px;align-items:center;margin:9px 0"><button class="device-link" onclick="openDevicePage(${x.id})">${esc(x.label||x.ip||x.mac||'Unknown')}</button><div style="height:9px;background:#edf2f7;border-radius:9px;overflow:hidden"><div style="height:100%;width:${Math.max(2,(Number(x.activity_events)||0)/mx*100)}%;background:#1976e8"></div></div><div class="muted">${Number(x.activity_events)||0}</div></div>`).join(''):'<div class="empty">No client activity has been observed yet.</div>';
...
async function boot(){try{if(!getCookie('godseye_csrf'))await fetch('/api/v1/auth/csrf');ME=await json('/api/v1/auth/me')}catch(e){showLogin();return}whoami.textContent=ME.username+' ('+ME.role+')';const tu=document.getElementById('topUser');if(tu)tu.textContent=ME.username;scanBtn.style.display=['admin','operator'].includes(ME.role)?'inline-block':'none';['navRules','navUsers'].forEach(id=>{document.getElementById(id).style.display=ME.role==='admin'?'':'none'});document.getElementById('navAudit').style.display=['admin','auditor'].includes(ME.role)?'':'none';const pwBar=document.getElementById('pwReminderBar');if(ME.password_change_reminder_days!==undefined){pwBar.textContent='⚠ Set a new password within '+ME.password_change_reminder_days+' day(s) — click here to do it now.';pwBar.style.display='block'}else{pwBar.style.display='none'}showApp();await load();await loadUsers();await loadAudit();await loadSecurity();await loadRules();await loadFindings();const rawHash=(location.hash||'#overview').slice(1);const dm=rawHash.match(/^device-(\d+)$/);if(dm)openDevicePage(Number(dm[1]),false);else showView(document.getElementById('view-'+rawHash)?rawHash:'overview',false);setInterval(()=>{if(document.getElementById('view-overview').style.display!=='none')load()},10000)}
