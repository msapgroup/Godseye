...
function v430RenderServices(settings,checks){const el=document.getElementById('v430Services');if(!el)return;const arr=Array.isArray(settings)?settings:[];const recent=Array.isArray(checks)?checks:[];if(!arr.length){el.innerHTML='<div class="empty">No monitors configured.</div>';return}el.innerHTML=arr.slice(0,6).map(m=>{const hit=recent.find(c=>String(c.monitor_id||c.setting_id||'')===String(m.id||''))||recent.find(c=>!c.monitor_id&&!c.setting_id&&String(c.kind||'')===String(m.kind||'')&&String(c.target||'')===String(m.target||''));const state=String(hit?.status||'unknown').toLowerCase();const ok=hit?['up','ok','healthy','online','running','success'].includes(state):m.enabled!==false;return `<div class="v430-service-row"><span>${esc(m.name||m.target||m.check_type||'Monitor')}</span><span><i class="v430-live-dot" style="background:${ok?'#41dfa1':'#ff5968'}"></i>${ok?'Running':'Attention'}</span><span>${esc(m.check_type||'Check')}</span></div>`}).join('')}
