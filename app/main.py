...
app = FastAPI(title="GODSEYE", version="1.9.0", lifespan=lifespan)
...
<div id="authOverlay" class="overlay" style="display:none">
  ...
  <div class="login-scene-footer">GODSEYE v1.9.0<br>Network Intelligence · Device Correlation · Smart Alerts</div>
</div>
...
async function loadAudit(){
  ...
  try{
    ...
    if(tbody)tbody.innerHTML=rows.length?rows.map(x=>`<tr><td>${x.created_at?esc(new Date(x.created_at).toLocaleString()):'—'}</td><td>${esc(x.actor)}</td><td><span class="pill">${esc(x.action)}</span></td><td>${esc(x.target||'—')}</td><td>${esc(x.details||'')}</td><td>${esc(x.ip||'—')}</td></tr>`).join(''):'<tr><td colspan="6" class="empty">No audit entries yet.</td></tr>';
...
async function loadInventory(){
  ...
  try{const q=new URLSearchParams();if(se?.value)q.set('search',se.value);if(st?.value)q.set('status',st.value);const response=await json('/api/v1/devices?'+q.toString()),d=Array.isArray(response)?response:[];if(ce)ce.textContent=d.length+' devices';tbody.innerHTML=d.length?d.map(x=>`<tr onclick="goDevice(${x.id})" style="cursor:pointer"><td><div class="device-name"><span class="device-icon">●</span><div><button class="link name" onclick="event.stopPropagation();goDevice(${x.id})">${esc(x.name||x.hostname||'Unknown device')}</button><div class="muted">${esc(x.device_type||'Unclassified')}</div></div></div></td><td>${esc(x.ip||'—')}</td><td>${esc(x.mac||'—')}</td><td>${esc(x.vendor||'—')}</td><td>${esc(x.device_type||'—')}</td><td>${x.last_seen?esc(new Date(x.last_seen).toLocaleString()):'—'}</td><td><span class="status-badge">${esc(x.status||'unknown')}</span></td><td><button class="secondary" onclick="event.stopPropagation();goDevice(${x.id})">Details</button></td></tr>`).join(''):'<tr><td colspan="8" class="empty">No devices discovered yet. Click Scan Now to discover the network.</td></tr>'}catch(e){tbody.innerHTML='<tr><td colspan="8" class="empty">Unable to load devices: '+esc(e.message)+'</td></tr>'}
}
