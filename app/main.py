...
<div id="authOverlay" class="overlay" style="display:none">
  ...
  <div class="login-scene-footer">GODSEYE v2.5.0<br>Network Intelligence · Device Correlation · Smart Alerts</div>
</div>
...
const DEVICE_ICON_CATEGORIES={auto:'other',router:'network',switch:'network','access-point':'network',firewall:'network',modem:'network',nas:'network','network-storage':'network','patch-panel':'network',pc:'computers',laptop:'computers',server:'computers',printer:'computers',phone:'computers','voip-phone':'computers',tablet:'computers',camera:'security',tv:'home','game-console':'home',iot:'home',other:'other'};
let ACTIVE_DEVICE_ICON_CATEGORY='network';
...
function renderDeviceIconPicker(selected){const p=document.getElementById('deviceIconPicker');if(!p)return;const keys=DEVICE_ICON_KEYS.filter(k=>DEVICE_ICON_CATEGORIES[k]===ACTIVE_DEVICE_ICON_CATEGORY);p.innerHTML=keys.map(k=>`<button type="button" class="device-icon-choice ${k===selected?'selected':''}" data-icon-key="${k}" onclick="selectDeviceIcon('${k}')"><img src="/assets/device-icons/${k==='auto'?'other':k}.svg" alt=""><span>${esc(DEVICE_ICON_LABELS[k])}</span></button>`).join('')}
...
function updateDeviceIconSummary(){const key=deviceIconKey.value||'auto';const img=document.getElementById('deviceIconCurrentPreview');if(!img)return;img.src=deviceIconData.value||('/assets/device-icons/'+(key==='auto'?'other':key)+'.svg')}
