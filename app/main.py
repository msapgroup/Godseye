...
<section class="panel"><div class="table-head"><div><h2>Per-Device Traffic Collection</h2><div class="muted">Choose exactly where GODSEYE gets device byte counters. The selected source is recorded with every sample and report.</div></div><span id="trafficCollectionState" class="pill">Loading…</span></div>
<div style="padding:16px" class="grid">
<label>Collection mode<select id="trafficMode" class="filter" onchange="renderTrafficModeFields()"><option value="unifi">UniFi / Controller</option><option value="snmp">SNMP</option><option value="span">SPAN / Mirror</option><option value="inline">Inline / Gateway</option></select></label>
...
<label>Sample interval (seconds)<input id="trafficInterval" class="input" type="number" min="10" max="3600" value="30"></label>
<label>Collection<select id="trafficEnabled" class="filter"><option value="0">Disabled</option><option value="1">Enabled</option></select></label>
<div style="align-self:end"><button class="primary" data-admin-only onclick="saveTrafficCollection()">Save Traffic Source</button> <button class="secondary" data-admin-only onclick="collectTrafficNow()">Collect Now</button></div>
</div>
<div id="trafficModeHelp" class="panel-subtle"></div>
<div style="overflow:auto"><table><thead><tr><th>Device</th><th>Source</th><th>Download</th><th>Upload</th><th>Peak Download</th><th>Peak Upload</th><th>Last Sample</th></tr></thead><tbody id="trafficDeviceRows"><tr><td colspan="7" class="empty">No per-device traffic samples yet.</td></tr></tbody></table></div>
</section>
...
function renderTrafficModeFields(){
 ...
 if(trafficConfigPayload){const cap=trafficConfigPayload.mode_capabilities?.[mode]||{};const label=trafficConfigPayload.modes?.[mode]||mode;trafficModeHelp.textContent=label+': '+(cap.description||'')+' '+(cap.requires?'Requires: '+cap.requires:'')}
}
function renderTrafficCollection(payload,usage){
 ...
 const state=c.enabled?(c.last_status||'configured'):'disabled';trafficCollectionState.textContent=state.replaceAll('_',' ').toUpperCase();trafficCollectionState.className='pill '+(state==='success'?'online':state==='failed'?'offline':'');
 renderTrafficModeFields();
 const rows=usage.devices||[];trafficDeviceRows.innerHTML=rows.length?rows.map(x=>`<tr><td>${esc(x.label)}</td><td>${esc(x.source_mode)} · ${esc(x.confidence)}</td><td>${fmtBytes(x.rx_bytes)}</td><td>${fmtBytes(x.tx_bytes)}</td><td>${fmtRate(x.peak_rx_bps)}</td><td>${fmtRate(x.peak_tx_bps)}</td><td>${x.last_sample?esc(new Date(x.last_sample).toLocaleString()):'—'}</td></tr>`).join(''):'<tr><td colspan="7" class="empty">No per-device traffic samples yet. Configure a source above; reports will not invent client usage when counters are unavailable.</td></tr>';
 applyRoleVisibility();
}
...
trafficMode?.addEventListener?.('change',()=>{if(!trafficConfigPayload)return;const ints=(trafficConfigPayload.integrations||[]).filter(x=>x.kind===trafficMode.value);trafficIntegration.innerHTML=ints.map(x=>`<option value="${x.id}">${esc(x.name)} — ${esc(x.target)}</option>`).join('')||'<option value="">No matching integration configured</option>';renderTrafficModeFields()});
