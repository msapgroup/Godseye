...
async function runCyberHostCheck(){const host=document.getElementById('cyberHost').value.trim(),o=document.getElementById('cyberHostOut');if(!host){o.textContent='Enter an authorized host.';return}o.textContent='Running diagnostics…';try{o.textContent=JSON.stringify(await json('/api/v1/diagnostics/host?host='+encodeURIComponent(host)),null,2)}catch(e){o.textContent='Host check failed: '+e.message}}
async function runCyberDns(){const host=document.getElementById('cyberHost').value.trim(),o=document.getElementById('cyberHostOut');if(!host){o.textContent='Enter a host.';return}try{o.textContent=JSON.stringify(await json('/api/v1/tools/dns?host='+encodeURIComponent(host)),null,2)}catch(e){o.textContent='DNS lookup failed: '+e.message}}
const VIEW_LOADERS={
