...
<button class="navitem" data-view="health" onclick="showView('health')"><span class="navicon">♥</span><span>System Health</span></button>
...
function showView(name){document.querySelectorAll('.view').forEach(v=>v.style.display='none');const target=document.getElementById('view-'+name);if(target)target.style.display='';document.querySelectorAll('.navitem').forEach(b=>b.classList.remove('active'));const btn=document.querySelector('.navitem[data-view="'+name+'"]');if(btn)btn.classList.add('active');if(name==='users'&&ME&&ME.role==='admin')loadUsers();if(name==='audit'&&ME&&['admin','auditor'].includes(ME.role))loadAudit();if(name==='rules')loadRules();if(name==='security')loadSecurity()}
...
async function boot(){try{if(!getCookie('godseye_csrf'))await fetch('/api/v1/auth/csrf');ME=await json('/api/v1/auth/me')}catch(e){showLogin();return}whoami.textContent=ME.username+' ('+ME.role+')';const tu=document.getElementById('topUser');if(tu)tu.textContent=ME.username;scanBtn.style.display=['admin','operator'].includes(ME.role)?'inline-block':'none';['navRules','navUsers'].forEach(id=>{document.getElementById(id).style.display=ME.role==='admin'?'':'none'});document.getElementById('navAudit').style.display=['admin','auditor'].includes(ME.role)?'':'none';const pwBar=document.getElementById('pwReminderBar');if(ME.password_change_reminder_days!==undefined){pwBar.textContent='⚠ Set a new password within '+ME.password_change_reminder_days+' day(s) — click here to do it now.';pwBar.style.display='block'}else{pwBar.style.display='none'}showApp();await load();await loadUsers();await loadAudit();await loadSecurity();await loadRules();await loadFindings();setInterval(load,10000)}
...
def tool_ping(req: ToolTargetRequest, user=Depends(get_current_user)):
    from .tools import ping
    return ping(req.host)

@app.post(f"{router_prefix}/tools/traceroute")
def tool_traceroute(req: TraceRequest, user=Depends(get_current_user)):
...
def tool_port_scan(req: PortScanRequest, user=Depends(get_current_user)):
...
def tool_device_info(req: ToolTargetRequest, user=Depends(get_current_user)):
