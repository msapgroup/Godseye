...
def windows_agent_heartbeat(req: WindowsAgentHeartbeatRequest, request: Request, agent=Depends(_agent_auth)):
    ...
    with db() as c:
        ...
        for r in pending:
            ...
            if finding:
                commands.append({"recheck_id":r["id"],"finding":dict(finding)})
    return {"ok":True,"server_time":ts,"channels":json.loads(fresh["channels_json"]),"poll_interval_seconds":fresh["poll_interval_seconds"],"enabled":bool(fresh["enabled"]),"rechecks":commands}
...
def windows_agent_package(user=Depends(require_admin)):
    ...
    if not path.exists(): raise HTTPException(404,"Windows Agent package is not installed")
    return FileResponse(path,media_type="application/zip",filename="GODSEYE-Windows-Agent.zip")
...
def event_findings(status: str | None=None, severity: str | None=None, computer: str | None=None, limit: int=500, user=Depends(get_current_user)):
    with db() as c:
        q="SELECT * FROM event_findings WHERE 1=1";params=[]
        if status: q+=" AND status=?";params.append(status)
        if severity: q+=" AND severity=?";params.append(severity)
        if computer: q+=" AND lower(computer_name) LIKE ?";params.append("%"+computer.lower()+"%")
        q+=" ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END,last_seen DESC LIMIT ?"
...
def ticket_create(req: TicketCreateRequest, request: Request, user=Depends(require_permission("operate"))):
    ...
    with db() as c:
        cur=c.execute("""INSERT INTO tickets(title,description,status,priority,assignee,linked_type,linked_id,device_name,due_at,created_by,created_at,updated_at)
                         VALUES(?,?,'open',?,?,?,?,?,?,?,?,?)""",
                      (req.title.strip()[:240],req.description.strip()[:8000],req.priority,req.assignee.strip()[:160],req.linked_type.strip()[:40],
...
def ticket_update(ticket_id: int, req: TicketUpdateRequest, request: Request, user=Depends(require_permission("operate"))):
    with db() as c:
        ...
        assignee=req.assignee.strip()[:160] if req.assignee is not None else d["assignee"]
...
<div class="event-findings-page">
  <div class="hero event-findings-hero"><div><h1>Event Findings</h1><div class="muted">Windows Critical, Error, and Warning events converted into explainable findings with remediation guidance.</div></div><div class="actions"><button class="primary admin-only" onclick="openWindowsAgentModal()">▣ Windows Agents</button><button class="secondary admin-only" onclick="openWindowsSourceModal()">⚙ WinRM Sources</button><button class="secondary operate-only" onclick="pollAllWindowsSources()">⟳ Pull Events Now (WinRM)</button></div></div>
 ...
 <div class="modal-card windows-agent-dialog" role="dialog" aria-modal="true">
  ...
  <div class="windows-agent-body">
    <div class="agent-onboarding">
      <div><b>Agent enrollment</b><div class="muted">Generate a one-time enrollment token, download the agent package, then run the installer as Administrator on the Windows computer.</div></div>
      <div class="actions"><button class="secondary" type="button" onclick="downloadWindowsAgentPackage()">↓ Download Agent</button><button class="primary admin-only" type="button" onclick="createWindowsAgentEnrollment()">＋ Create Enrollment Token</button></div>
    </div>
    <div id="windowsAgentEnrollment" class="agent-enrollment-result" style="display:none">
      ...
      <div class="muted">PowerShell install command:</div><pre id="windowsAgentInstallCommand" class="agent-command"></pre>
...
<div class="ticket-page">
  ...
  <section class="panel"><div class="table-head"><div><h2>Tickets</h2><div class="muted">Open a ticket to add notes, change status, or schedule it on the Calendar.</div></div><div><select id="ticketStatusFilter" class="filter" onchange="loadTickets()"><option value="">All status</option><option value="open">Open</option><option value="assigned">Assigned</option><option value="in_progress">In Progress</option><option value="waiting">Waiting</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select></div></div>
  <div style="overflow:auto"><table><thead><tr><th>Ticket</th><th>Priority</th><th>Title</th><th>Affected</th><th>Assignee</th><th>Status</th><th>Due / Calendar</th><th>Updated</th><th>Action</th></tr></thead><tbody id="ticketRows"></tbody></table></div></section>
</div>
</div>
...
<div id="ticketEditorModal" class="modal" style="display:none" onclick="if(event.target===this)closeTicketEditor()">
 <div class="modal-card ticket-dialog" role="dialog" aria-modal="true">
  ...
  <div class="ticket-editor-grid">
    ...
    <label>Assignee<input id="ticketAssignee" class="input" placeholder="Administrator / technician"></label>
...
function renderWindowsAgents(){
 ...
 root.innerHTML=WINDOWS_AGENTS.length?WINDOWS_AGENTS.map(x=>`<div class="windows-agent-row"><span class="windows-agent-icon">W</span><div><b>${esc(x.computer_name||x.hostname||'Windows Agent')}</b><small><span class="agent-status ${esc(x.status||'enrolled')}"><span class="agent-status-dot"></span>${esc(x.status||'enrolled')}</span> · ${esc(x.ip_address||'no IP')} · ${esc(x.os_version||'Windows')} · Agent ${esc(x.agent_version||'—')} · ${x.open_findings||0} open finding(s)</small><small>Last heartbeat: ${x.last_heartbeat_at?esc(new Date(x.last_heartbeat_at).toLocaleString()):'never'} · Channels: ${(x.channels||[]).map(esc).join(', ')} · every ${x.poll_interval_seconds||60}s${x.last_error?' · '+esc(x.last_error):''}</small></div><div class="actions">${x.revoked_at?'':`<button class="secondary admin-only" onclick="configureWindowsAgent(${x.id})">Configure</button><button class="danger admin-only" onclick="revokeWindowsAgent(${x.id})">Revoke</button>`}</div></div>`).join(''):'<div class="empty">No Windows Agents enrolled.</div>';
 applyRoleVisibility();
}
async function createWindowsAgentEnrollment(){
 ...
 try{
   ...
   const allowHttp=location.protocol==='http:'?' -AllowHttp':'';
   windowsAgentInstallCommand.textContent=`.\\Install-GODSEYEAgent.ps1 -ServerUrl "${location.origin}" -EnrollmentToken "${r.enrollment_token}"${allowHttp}`;
...
async function loadEventFindings(){
 ...
 try{
   const [rows,allOpen,resolved]=await Promise.all([
     json('/api/v1/event-findings?'+new URLSearchParams({status,severity}).toString()),
   ...
   eventFindingRows.innerHTML=EVENT_FINDINGS.length?EVENT_FINDINGS.map(x=>`<tr>
     ...
     <td><button class="link" onclick="openEventFinding(${x.id})">Open</button>${x.status==='open'?` <button class="link operate-only" onclick="recheckEventFinding(${x.id})">Recheck</button> <button class="link operate-only" onclick="createTicketFromEventFinding(${x.id})">Create Ticket</button>`:''}</td>
   </tr>`).join(''):'<tr><td colspan="8" class="empty">No Event Findings match this filter.</td></tr>';
   applyRoleVisibility();
   await Promise.all([loadWindowsAgents(),loadWindowsSources()]);
 }catch(e){eventFindingRows.innerHTML='<tr><td colspan="8" class="empty">Event Findings unavailable: '+esc(e.message)+'</td></tr>'}
}
...
function closeTicketEditor(){ticketEditorModal.style.display='none';document.body.style.overflow='';CURRENT_TICKET=null}
