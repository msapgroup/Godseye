...
EYE_LOGO = '<img class="eye-logo godseye-shield-g-eye" src="/assets/godseye-approved.png" alt="GODSEYE shield G cyber eye logo">'
...
@app.get("/assets/kb.css", include_in_schema=False)
def kb_css_asset():
    return FileResponse(BASE_DIR / "app" / "assets" / "kb.css", media_type="text/css")

@app.get("/assets/kb.js", include_in_schema=False)
def kb_js_asset():
    return FileResponse(BASE_DIR / "app" / "assets" / "kb.js", media_type="application/javascript")

@app.get("/assets/kb-outlook-{name}.svg", include_in_schema=False)
def kb_outlook_image(name: str):
    if name not in {"settings", "account", "verify"}:
        raise HTTPException(404)
    return FileResponse(BASE_DIR / "app" / "assets" / f"kb-outlook-{name}.svg", media_type="image/svg+xml")
