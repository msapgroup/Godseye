...
def _event_finding_public(row):
    ...
    from .event_ticketing import classify_windows_event, trusted_event_fix_links
    # Existing findings created before the Defrag rule gained specific guidance
    # should show the improved fix without waiting for another event to arrive.
    if d.get("event_id")==264 and "defrag" in (d.get("provider") or "").lower() and d.get("category")=="Windows":
        fix=classify_windows_event(d)
        d.update(category=fix["category"],title=fix["title"],recommendation=fix["recommendation"],suggested_actions=fix["actions"])
    d["trusted_fix_links"]=trusted_event_fix_links(d)
    return d
...
def event_finding_suggested_fix(finding_id: int, user=Depends(get_current_user)):
    ...
    return {"id":finding_id,"title":d["title"],"recommendation":d["recommendation"],"actions":d["suggested_actions"],"trusted_fix_links":d["trusted_fix_links"],"event_id":d["event_id"],"provider":d["provider"],"computer_name":d["computer_name"]}
...
.cyber-tool-card-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(315px,1fr));gap:14px;align-items:stretch;margin-bottom:16px}
.cyber-tool-card{position:relative!important;min-height:305px;display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:9px!important;padding:17px!important;border:1px solid #cbd9e8!important;border-radius:14px!important;background:linear-gradient(145deg,#fff,#f5f9fd)!important;box-shadow:0 8px 20px rgba(16,48,82,.07)!important;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease!important}
...
.cyber-card-head{display:grid;grid-template-columns:38px minmax(0,1fr) auto;gap:10px;align-items:start}
...
.cyber-tool-card p{margin:3px 0 0!important;min-height:30px;color:#647a91!important;font-size:10px!important;line-height:1.45}
.cyber-card-form{display:grid;grid-template-columns:minmax(0,1fr) 108px;gap:8px;margin-top:3px}.cyber-card-form.one{grid-template-columns:1fr}.cyber-card-form label{font-size:9px;color:#62778f}.cyber-card-form .input{margin-top:4px;width:100%;font-size:10px;padding:8px}.cyber-card-form input[type=file]{padding:6px;font-size:9px}
.cyber-card-buttons{display:flex;gap:7px;flex-wrap:wrap}.cyber-tool-card .primary,.cyber-tool-card .secondary{font-size:9px!important;padding:7px 10px!important;border-radius:8px!important}
.cyber-card-result{margin:0;min-height:66px;max-height:150px;overflow:auto;white-space:pre-wrap;background:#eef5fb;border:1px solid #d5e4f0;border-radius:9px;padding:9px;font-size:9px;line-height:1.4;color:#29445e}.cyber-card-result.running{color:#0d74f5}.cyber-card-result.failed{color:#ba3040}
.cyber-card-actions{display:none;gap:6px;flex-wrap:wrap;margin-top:auto}.cyber-card-actions.visible{display:flex}.cyber-card-actions a,.cyber-card-actions button{font-size:8px!important;padding:5px 8px!important}
.cyber-tool-card button:disabled{opacity:.55;cursor:not-allowed}
.cyber-tool-status{white-space:nowrap;font-size:8px!important;font-weight:800!important;letter-spacing:.03em;text-transform:uppercase;border-radius:99px;padding:4px 7px;background:#eef4f9}
.cyber-tool-status.ready{color:#16865a!important}
.cyber-tool-status.unavailable{color:#a16b09!important}.cyber-tool-status.running{color:#0d74f5!important}
.cyber-history-grid{display:grid;grid-template-columns:minmax(0,2fr) minmax(260px,1fr);gap:14px}.cyber-history-list,.cyber-schedule-list{display:grid;gap:7px}.cyber-history-row{display:grid;grid-template-columns:105px minmax(0,1fr) auto;gap:10px;align-items:center;border-top:1px solid var(--line,#dce7f1);padding:8px 0;font-size:9px}.cyber-history-row:first-child{border-top:0}.cyber-history-tool{font-weight:800}.cyber-history-meta{color:#6c8198}.cyber-history-state{font-weight:800;color:#16865a}.cyber-history-state.failed{color:#c13c4b}.cyber-schedule-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center;border-top:1px solid var(--line,#dce7f1);padding:8px 0;font-size:9px}.cyber-schedule-row:first-child{border-top:0}
...
html[data-theme="dark"] .cyber-card-result{background:#0b1824;border-color:#29445f;color:#c1d4e5}html[data-theme="dark"] .cyber-card-form label,html[data-theme="dark"] .cyber-history-meta{color:#9fb3c7}
@media(max-width:900px){.cyber-history-grid{grid-template-columns:1fr}}@media(max-width:520px){.cyber-tool-card-grid{grid-template-columns:1fr}.cyber-card-form{grid-template-columns:1fr}.cyber-history-row{grid-template-columns:1fr}}
...
.event-fix-resources{display:grid;gap:7px;margin-top:12px}.event-fix-resources h4{margin:0 0 2px;font-size:10px}.event-fix-resource{display:flex;align-items:center;justify-content:space-between;gap:10px;border:1px solid #d6e4f2;border-radius:8px;padding:9px 11px;background:#f4f9ff;color:#176fc8;text-decoration:none;font-size:10px;font-weight:750}.event-fix-resource small{font-size:8px;font-weight:600;color:#607e9a}.event-fix-resource:hover,.event-fix-resource:focus-visible{border-color:#70a9dd;background:#eaf4ff;text-decoration:underline}
...
html[data-theme="dark"] .event-fix-resource{background:#10253b;border-color:#345b80;color:#a8d5ff}html[data-theme="dark"] .event-fix-resource small{color:#aebdd0}html[data-theme="dark"] .event-fix-resource:hover,html[data-theme="dark"] .event-fix-resource:focus-visible{background:#193750;border-color:#70a9dd}
...
.remote-layout{grid-template-columns:minmax(280px,.38fr) minmax(600px,1.62fr)!important;gap:13px!important}.remote-screen-wrap{height:clamp(320px,calc(100vh - 285px),760px)!important;min-height:0!important;display:flex!important;align-items:center!important;justify-content:center!important;overflow:hidden!important}.remote-screen{width:auto!important;height:auto!important;max-width:100%!important;max-height:100%!important;object-fit:contain!important;display:block!important}
/* Auth */
...
.v430-kpi-grid{grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:12px!important;margin-bottom:12px!important}
.v430-kpi-grid .statcard{min-height:102px!important;padding:15px 16px!important;display:grid!important;grid-template-columns:48px minmax(0,1fr) 58px!important;align-items:center!important;gap:11px!important;position:relative!important;overflow:hidden!important}
.v430-kpi-grid .staticon{width:43px!important;height:43px!important;border-radius:10px!important;background:#102e47!important;color:#42bcff!important;display:grid!important;place-items:center!important}
.v430-kpi-grid .staticon.red{background:#391a27!important;color:#ff6479!important}.v430-kpi-grid .staticon.purple{background:#2d2448!important;color:#b391ff!important}
.v430-kpi-grid .statmeta{font-size:9px!important;text-transform:none!important;color:#87a2b5!important;letter-spacing:0!important}.v430-kpi-grid .statnum{font-size:26px!important;line-height:1.05!important;color:#f5fbff!important;margin-top:4px!important}.v430-kpi-grid .v430-kpi-sub{font-size:8px!important;margin-top:5px!important}
.v430-kpi-spark{height:36px;width:58px;opacity:.8;background-repeat:no-repeat;background-position:center;background-size:contain}.spark-blue{background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 60 30' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 25 9 22 16 23 25 12 33 17 41 8 48 12 59 4' fill='none' stroke='%2316a5ff' stroke-width='2'/%3E%3C/svg%3E")}.spark-red{background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 60 30' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 22 10 19 17 24 25 11 34 16 43 8 52 13 59 6' fill='none' stroke='%23ff6278' stroke-width='2'/%3E%3C/svg%3E")}.spark-cyan{background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 60 30' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 24 10 20 19 21 28 15 36 18 45 10 53 12 59 5' fill='none' stroke='%2328d1ff' stroke-width='2'/%3E%3C/svg%3E")}.spark-purple{background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 60 30' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 22 8 18 16 20 24 13 32 16 42 8 50 10 59 3' fill='none' stroke='%239b6cff' stroke-width='2'/%3E%3C/svg%3E")}
.v430-dashboard-main{grid-template-columns:minmax(0,1.55fr) minmax(380px,.95fr)!important;gap:12px!important}.v430-dashboard-main .panel{min-height:320px!important;margin:0!important}.v430-traffic-panel svg{height:215px!important;margin:8px 0 0!important}.filter-chip{padding:5px 8px;border:1px solid #274b64;border-radius:6px;background:#091a29;color:#9eb7c8;font-size:8px}.traffic-legend{display:flex;align-items:center;gap:16px;padding:6px 16px 2px;color:#8da8ba;font-size:8px}.traffic-legend i{width:7px;height:7px;border-radius:50%;display:inline-block;margin-right:5px}.traffic-legend .cyan{background:#16a5ff}.traffic-legend .purple{background:#9b6cff}.traffic-legend #trafficNow{margin-left:auto;color:#c6ddea}
.dashboard-alerts .activity-list{padding:4px 13px 10px!important}.dashboard-alerts .activity-row{min-height:46px!important;padding:8px 4px!important;border-bottom:1px solid #18374d!important}.dashboard-alerts .activity-title{font-size:10px!important;color:#edf7ff!important;font-weight:700}.dashboard-alerts .activity-sub{font-size:8px!important;color:#839eb1!important}
.v430-dashboard-bottom{grid-template-columns:minmax(0,1.25fr) minmax(270px,.8fr) minmax(270px,.8fr)!important;gap:12px!important;margin-top:12px!important}.v430-dashboard-bottom>.panel{min-height:280px!important;margin:0!important}
.godseye-map-scene{height:190px;position:relative;overflow:hidden;background:radial-gradient(circle at 50% 50%,rgba(16,74,115,.22),transparent 50%),#071725;border-bottom:1px solid #18384e}.map-world{position:absolute;inset:0;background:url('/assets/dashboard-map-reference.png') center 44%/cover no-repeat;opacity:.28;filter:saturate(.5) hue-rotate(160deg)}.map-hub{position:absolute;left:48%;top:48%;transform:translate(-50%,-50%);width:54px;height:54px;border-radius:50%;display:grid;place-items:center;background:#0b2338;border:1px solid #32b8ff;box-shadow:0 0 20px rgba(41,184,255,.28);z-index:4}.map-hub .eye-logo{width:42px!important;height:42px!important}.map-hub small{position:absolute;top:58px;white-space:nowrap;color:#d9f2ff;font-size:8px}.map-site{position:absolute;z-index:3;display:flex;align-items:center;gap:4px;color:#9db8ca;font-size:7px}.map-site i{width:9px;height:9px;border-radius:50%;background:#35d695;box-shadow:0 0 0 4px rgba(53,214,149,.12)}.map-site.s1{left:13%;top:27%}.map-site.s2{right:15%;top:28%}.map-site.s3{right:10%;bottom:23%}.map-site.s4{left:18%;bottom:19%}.map-link{position:absolute;left:48%;top:49%;height:1px;background:repeating-linear-gradient(90deg,#2c9fdf 0 4px,transparent 4px 8px);transform-origin:left center;opacity:.6}.map-link.a{width:34%;transform:rotate(-151deg)}.map-link.b{width:34%;transform:rotate(-29deg)}.map-link.c{width:38%;transform:rotate(25deg)}.map-link.d{width:34%;transform:rotate(147deg)}.map-meta-strip{display:grid!important;grid-template-columns:repeat(4,1fr)!important;padding:9px 12px!important;background:#081824!important}.map-meta-strip span{text-align:center;font-size:7px;color:#7995a9}.map-meta-strip b{display:block;font-size:14px;color:#eaf7ff}
.v430-service-row,.v430-ticket-row{min-height:42px!important;border-color:#17374d!important;padding:8px 13px!important;font-size:9px!important}
@media(max-width:1100px){.v430-dashboard-main{grid-template-columns:1fr!important}.v430-kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}.v430-dashboard-bottom{grid-template-columns:1fr!important}}
@media(max-width:650px){.v430-kpi-grid{grid-template-columns:1fr!important}.v430-kpi-grid .statcard{grid-template-columns:44px 1fr 48px!important}}
</style></head>
<body>
...
<div class="hero dashboard-hero"><div><h1 id="dashboardGreeting">Dashboard</h1><div class="muted">Real-time view of your environment, security posture, and operations.</div><div class="dashboard-statusline"><span class="status-pill" id="dashScannerState">Scanner checking…</span><span id="dashRefreshState">Waiting for first refresh</span></div></div><div class="v430-dashboard-clock"><span id="v430Date">Today</span><b id="v430Clock">--:--</b></div></div>
<div class="cards v430-kpi-grid">
<button type="button" class="card statcard stat-action" onclick="openDashboardSummary('devices')"><div class="staticon"><svg viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="12" rx="2"/><path d="M8 21h8M12 17v4"/></svg></div><div><div class="statmeta">Total Devices</div><div class="statnum" id="total">—</div><div class="v430-kpi-sub green" id="v430DeviceSub">Network inventory</div></div><span class="v430-kpi-spark spark-blue"></span></button>
<button type="button" class="card statcard stat-action" onclick="openDashboardSummary('issues')"><div class="staticon red"><svg viewBox="0 0 24 24"><path d="M12 3 20 6v6c0 5-3.2 8-8 9-4.8-1-8-4-8-9V6Z"/><path d="M12 8v5M12 16h.01"/></svg></div><div><div class="statmeta">Active Findings</div><div class="statnum red" id="unknown">—</div><div class="v430-kpi-sub red">Needs review</div></div><span class="v430-kpi-spark spark-red"></span></button>
<button type="button" class="card statcard stat-action" onclick="showView('monitoring')"><div class="staticon v430-cyan"><svg viewBox="0 0 24 24"><ellipse cx="12" cy="5" rx="8" ry="3"/><path d="M4 5v7c0 1.7 3.6 3 8 3s8-1.3 8-3V5M4 12v7c0 1.7 3.6 3 8 3s8-1.3 8-3v-7"/></svg></div><div><div class="statmeta">Monitored Services</div><div class="statnum" id="dashboardServiceCount">—</div><div class="v430-kpi-sub green">Active checks</div></div><span class="v430-kpi-spark spark-cyan"></span></button>
<button type="button" class="card statcard stat-action" onclick="showView('tickets')"><div class="staticon purple"><svg viewBox="0 0 24 24"><path d="M4 6h16v4a2 2 0 0 0 0 4v4H4v-4a2 2 0 0 0 0-4Z"/><path d="M12 8v8"/></svg></div><div><div class="statmeta">Open Tickets</div><div class="statnum" id="dashboardTicketCount">—</div><div class="v430-kpi-sub">Work queue</div></div><span class="v430-kpi-spark spark-purple"></span></button>
</div>
<div class="v430-dashboard-main dashboard-grid">
<section class="panel traffic v430-traffic-panel"><div class="table-head"><h2>Network Traffic (Last 24 Hours)</h2><div class="v430-panel-tools"><span class="filter-chip">All Sites</span><span class="filter-chip">Last 24 Hours</span></div></div><svg id="trafficSvg" viewBox="0 0 760 190" preserveAspectRatio="none" aria-label="Network traffic chart"><defs><linearGradient id="rxFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="#168fff" stop-opacity=".28"/><stop offset="1" stop-color="#168fff" stop-opacity="0"/></linearGradient></defs><g class="v430-grid-lines" stroke="#20354b" stroke-width="1"><line x1="45" y1="25" x2="745" y2="25"/><line x1="45" y1="65" x2="745" y2="65"/><line x1="45" y1="105" x2="745" y2="105"/><line x1="45" y1="145" x2="745" y2="145"/></g><g id="trafficLabels"></g><polyline id="trafficRx" points="" fill="none" stroke="#16a5ff" stroke-width="2.4"/><polyline id="trafficTx" points="" fill="none" stroke="#9a67ff" stroke-width="2.1"/></svg><div class="traffic-legend"><span><i class="cyan"></i>Inbound</span><span><i class="purple"></i>Outbound</span><span id="trafficNow"></span></div><div class="panel-subtle" id="trafficFoot">Traffic collector is warming up…</div></section>
...
<section class="panel dashboard-map-card"><div class="table-head"><h2>Network Map Overview</h2><button class="link" onclick="showView('network')">View Full Map →</button></div><div class="godseye-map-scene"><div class="map-world"></div><div class="map-link a"></div><div class="map-link b"></div><div class="map-link c"></div><div class="map-link d"></div><div class="map-site s1"><i></i><span>Seattle</span></div><div class="map-site s2"><i></i><span>London</span></div><div class="map-site s3"><i></i><span>Tokyo</span></div><div class="map-site s4"><i></i><span>Branch Office</span></div><div class="map-hub">__EYE_LOGO__<small>Headquarters</small></div></div><div class="v430-map-meta map-meta-strip"><span><b id="v430MapSites">0</b>Sites</span><span><b id="v430MapDevices">0</b>Devices</span><span><b class="red" id="v430MapOffline">0</b>Offline</span><span><b class="yellow" id="v430MapAlerts">0</b>Active Alerts</span></div></section>
...
<div hidden><span id="online"></span><span id="onlineTrend"></span><span id="systemHealthState"></span><span id="systemHealthTrend"></span><div id="v430DeviceDonut"></div><span id="v430DonutTotal"></span><span id="v430LegendOnline"></span><span id="v430LegendOffline"></span><span id="v430LegendReview"></span><div id="clientBars"></div><div id="clientMetricNote"></div><table aria-hidden="true"><tbody id="devices"></tbody></table><input id="search"><select id="status"><option value=""></option></select><select id="classification"><option value=""></option></select></div>
</div>
...
<div class="hero"><div><h1>Cyber Tools</h1><div class="muted">Operational defensive checks for systems and networks you are authorized to assess.</div></div><div class="actions"><button class="secondary" onclick="loadCyberTools()">↻ Refresh</button><span class="status">Bounded defensive checks</span></div></div>
<section class="cyber-tool-card-grid">
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">NET</div><div><h3>Network Exposure Scan</h3><p>Discover private hosts and inventory open services with bounded Nmap profiles.</p></div><span id="cyber-cap-network_exposure" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form"><label>Private host or network<input id="cyber-network-target" class="input" value="192.168.1.0/24"></label><label>Profile<select id="cyber-network-profile" class="input"><option value="quick">Quick</option><option value="standard" selected>Standard</option><option value="deep">Deep</option></select></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="network_exposure" onclick="runCyberTool('network_exposure')">Run scan</button><button class="secondary" onclick="scheduleCyberTool('network_exposure')">Schedule</button></div><pre id="cyber-result-network_exposure" class="cyber-card-result">Ready.</pre><div id="cyber-actions-network_exposure" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">EDR</div><div><h3>Endpoint Security Posture</h3><p>Review agent reachability, version, update checks, errors, and malware-scan state.</p></div><span id="cyber-cap-endpoint_posture" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form one"><label>Scope<input class="input" value="All enrolled Windows endpoints" disabled></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="endpoint_posture" onclick="runCyberTool('endpoint_posture')">Review posture</button><button class="secondary" onclick="scheduleCyberTool('endpoint_posture')">Schedule</button></div><pre id="cyber-result-endpoint_posture" class="cyber-card-result">Ready.</pre><div id="cyber-actions-endpoint_posture" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">TLS</div><div><h3>Web &amp; TLS Audit</h3><p>Validate HTTPS, certificate negotiation, redirects, and browser security headers.</p></div><span id="cyber-cap-web_tls" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form"><label>Website<input id="cyber-web-target" class="input" placeholder="https://portal.example.com"></label><label>Profile<select id="cyber-web-profile" class="input"><option value="quick">Quick</option><option value="standard" selected>Standard</option><option value="deep">Deep</option></select></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="web_tls" onclick="runCyberTool('web_tls')">Run audit</button><button class="secondary" onclick="scheduleCyberTool('web_tls')">Schedule</button></div><pre id="cyber-result-web_tls" class="cyber-card-result">Ready.</pre><div id="cyber-actions-web_tls" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">IOC</div><div><h3>Malware &amp; IOC Scan</h3><p>Classify an indicator and scan an optional file with ClamAV and YARA.</p></div><span id="cyber-cap-malware_ioc" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form one"><label>Hash, IP, or domain<input id="cyber-malware-target" class="input" placeholder="SHA-256, IP, or domain"></label><label>Optional file — 10 MB maximum<input id="cyber-malware-file" class="input" type="file"></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="malware_ioc" onclick="runCyberTool('malware_ioc')">Scan indicator / file</button></div><pre id="cyber-result-malware_ioc" class="cyber-card-result">Ready.</pre><div id="cyber-actions-malware_ioc" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">DNS</div><div><h3>DNS &amp; Email Security</h3><p>Inspect MX, SPF, DMARC, DKIM selector, CAA, address, and DNSSEC records.</p></div><span id="cyber-cap-dns_email" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form"><label>Domain<input id="cyber-dns-target" class="input" placeholder="example.com"></label><label>DKIM selector<input id="cyber-dns-selector" class="input" value="default"></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="dns_email" onclick="runCyberTool('dns_email')">Audit DNS</button><button class="secondary" onclick="scheduleCyberTool('dns_email')">Schedule</button></div><pre id="cyber-result-dns_email" class="cyber-card-result">Ready.</pre><div id="cyber-actions-dns_email" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">LIN</div><div><h3>Linux Security Audit</h3><p>Run a read-only Lynis assessment of the GODSEYE appliance.</p></div><span id="cyber-cap-linux_audit" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form one"><label>Profile<select id="cyber-linux-profile" class="input"><option value="quick">Quick</option><option value="standard" selected>Standard</option><option value="deep">Deep</option></select></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="linux_audit" onclick="runCyberTool('linux_audit')">Run Lynis</button><button class="secondary" onclick="scheduleCyberTool('linux_audit')">Schedule</button></div><pre id="cyber-result-linux_audit" class="cyber-card-result">Ready.</pre><div id="cyber-actions-linux_audit" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">IDS</div><div><h3>Network Threat Detection</h3><p>Summarize recent Suricata EVE alerts with source and destination evidence.</p></div><span id="cyber-cap-threat_detection" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form one"><label>History depth<select id="cyber-threat-profile" class="input"><option value="quick">25 alerts</option><option value="standard" selected>100 alerts</option><option value="deep">250 alerts</option></select></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="threat_detection" onclick="runCyberTool('threat_detection')">Review alerts</button><button class="secondary" onclick="scheduleCyberTool('threat_detection')">Schedule</button></div><pre id="cyber-result-threat_detection" class="cyber-card-result">Ready.</pre><div id="cyber-actions-threat_detection" class="cyber-card-actions"></div></article>
<article class="cyber-tool-card"><div class="cyber-card-head"><div class="cyber-tool-card-icon">PCAP</div><div><h3>Evidence Capture</h3><p>Capture a time- and packet-bounded PCAP on a validated interface.</p></div><span id="cyber-cap-evidence_capture" class="cyber-tool-status">Checking</span></div><div class="cyber-card-form"><label>Interface<select id="cyber-evidence-interface" class="input"><option value="">Loading…</option></select></label><label>Host filter<input id="cyber-evidence-target" class="input" placeholder="Optional IP"></label><label>Seconds<input id="cyber-evidence-seconds" class="input" type="number" value="10" min="1" max="15"></label><label>Packet limit<input id="cyber-evidence-count" class="input" type="number" value="100" min="1" max="500"></label></div><div class="cyber-card-buttons"><button class="primary" data-cyber-run="evidence_capture" onclick="runCyberTool('evidence_capture')">Capture evidence</button></div><pre id="cyber-result-evidence_capture" class="cyber-card-result">Ready.</pre><div id="cyber-actions-evidence_capture" class="cyber-card-actions"></div></article>
</section>
<section class="cyber-history-grid"><div class="panel"><div class="table-head"><div><h2>Scan history</h2><div class="muted">Every manual and scheduled run is retained with its result.</div></div></div><div id="cyberHistory" class="cyber-history-list"><div class="empty">Loading history…</div></div></div><div class="panel"><div class="table-head"><div><h2>Schedules</h2><div class="muted">Recurring checks use the same limits as manual scans.</div></div></div><div id="cyberSchedules" class="cyber-schedule-list"><div class="empty">Loading schedules…</div></div></div></section>
<section class="panel" style="margin-top:14px"><div class="table-head"><h2>Defensive scope</h2></div><div class="muted">Network scans are limited to private/link-local targets and 256 addresses. GODSEYE does not provide exploit delivery, credential attacks, persistence, or unrestricted command execution.</div></section>
</div>
...
async function loadEventFindings(){
 ...
 try{
   ...
   eventFindingRows.innerHTML=EVENT_FINDINGS.length?EVENT_FINDINGS.map(x=>`<tr>
     ...
     <td><button class="link" onclick="openEventFinding(${x.id})">Open</button> <button class="link" onclick="openEventFinding(${x.id},true)">Suggested Fix</button>${x.status==='open'?` <button class="link operate-only" onclick="recheckEventFinding(${x.id})">Recheck</button> <button class="link operate-only" onclick="createTicketFromEventFinding(${x.id})">Create Ticket</button>`:''} <button class="link delete-link admin-only" onclick="deleteEventFinding(${x.id})">Delete</button></td>
...
async function openEventFinding(id,focusFix=false){
 try{
   ...
   const resources=(x.trusted_fix_links||[]).filter(link=>/^https:\/\/(learn\.microsoft\.com|www\.google\.com)\//.test(link.url||'')).map(link=>`<a class="event-fix-resource" href="${esc(link.url)}" target="_blank" rel="noopener noreferrer"><span>${esc(link.label)} ↗</span><small>${esc(link.source)}</small></a>`).join('');
   ...
   eventFindingModalBody.innerHTML=`<div class="event-detail-grid">
    ...
    <div id="eventFindingSuggestedFix"><h3>Suggested Fix</h3><p class="muted">${esc(x.recommendation||'')}</p><div class="event-fix-list">${actions}</div>${resources?`<div class="event-fix-resources"><h4>Trusted troubleshooting guides</h4>${resources}</div>`:''}</div>
   ...
   if(focusFix)document.getElementById('eventFindingSuggestedFix')?.scrollIntoView({block:'start'});
...
async function saveTicket(){
 ...
 try{
  if(id){
    ...
    await json('/api/v1/tickets/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});closeTicketEditor();await loadTickets();
  }else{
...
const CYBER_TOOL_NAMES={network_exposure:'Network Exposure',endpoint_posture:'Endpoint Posture',web_tls:'Web & TLS',malware_ioc:'Malware & IOC',dns_email:'DNS & Email',linux_audit:'Linux Audit',threat_detection:'Threat Detection',evidence_capture:'Evidence Capture'};
const CYBER_LAST_RUN={};
function cyberInput(id){return(document.getElementById(id)?.value||'').trim()}
async function cyberFilePayload(){const file=document.getElementById('cyber-malware-file')?.files?.[0];if(!file)return{};if(file.size>10*1024*1024)throw new Error('The file must be 10 MB or smaller.');const encoded=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',',2)[1]||'');reader.onerror=()=>reject(new Error('Could not read the selected file.'));reader.readAsDataURL(file)});return{filename:file.name,file_base64:encoded}}
async function cyberToolPayload(tool){
 if(tool==='network_exposure')return{tool,target:cyberInput('cyber-network-target'),profile:cyberInput('cyber-network-profile'),options:{}};
 if(tool==='endpoint_posture')return{tool,target:'All enrolled endpoints',profile:'standard',options:{}};
 if(tool==='web_tls')return{tool,target:cyberInput('cyber-web-target'),profile:cyberInput('cyber-web-profile'),options:{}};
 if(tool==='malware_ioc')return{tool,target:cyberInput('cyber-malware-target'),profile:'standard',options:await cyberFilePayload()};
 if(tool==='dns_email')return{tool,target:cyberInput('cyber-dns-target'),profile:'standard',options:{selector:cyberInput('cyber-dns-selector')||'default'}};
 if(tool==='linux_audit')return{tool,target:'GODSEYE appliance',profile:cyberInput('cyber-linux-profile'),options:{}};
 if(tool==='threat_detection')return{tool,target:'Suricata EVE',profile:cyberInput('cyber-threat-profile'),options:{}};
 if(tool==='evidence_capture')return{tool,target:cyberInput('cyber-evidence-target'),profile:'standard',options:{interface:cyberInput('cyber-evidence-interface'),seconds:Number(cyberInput('cyber-evidence-seconds')||10),packet_count:Number(cyberInput('cyber-evidence-count')||100)}};
 throw new Error('Unknown Cyber Tool.');
}
function cyberResultText(run){let detail=JSON.stringify(run.result||{},null,2);if(detail.length>10000)detail=detail.slice(0,10000)+'\n… result shortened on screen; export retains the stored report.';return run.summary+'\n\n'+detail}
function renderCyberActions(tool,run){const el=document.getElementById('cyber-actions-'+tool);if(!el)return;CYBER_LAST_RUN[tool]=run;const download=run.result?.download_url?`<a class="secondary" href="${esc(run.result.download_url)}">Download PCAP</a>`:'';el.innerHTML=`<button class="secondary" onclick="createCyberFinding('${tool}')">Create finding</button><button class="secondary" onclick="createCyberTicket('${tool}')">Create ticket</button><a class="secondary" href="/api/v1/cyber-tools/runs/${run.id}/export">Export JSON</a>${download}`;el.classList.add('visible')}
async function runCyberTool(tool){const out=document.getElementById('cyber-result-'+tool),button=document.querySelector(`[data-cyber-run="${tool}"]`),badge=document.getElementById('cyber-cap-'+tool);if(out){out.className='cyber-card-result running';out.textContent='Running '+(CYBER_TOOL_NAMES[tool]||tool)+'…'}if(button)button.disabled=true;if(badge){badge.className='cyber-tool-status running';badge.textContent='Running'}try{const payload=await cyberToolPayload(tool),run=await json('/api/v1/cyber-tools/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(out){out.className='cyber-card-result'+(run.status==='failed'?' failed':'');out.textContent=cyberResultText(run)}renderCyberActions(tool,run);await loadCyberHistory()}catch(e){if(out){out.className='cyber-card-result failed';out.textContent='Run failed: '+e.message}}finally{if(button)button.disabled=false;await loadCyberCapabilities()}}
async function createCyberFinding(tool){const run=CYBER_LAST_RUN[tool];if(!run)return;try{const finding=await json('/api/v1/cyber-tools/runs/'+run.id+'/create-finding',{method:'POST'});alert('Finding #'+finding.id+' created.');await loadCyberHistory()}catch(e){alert('Could not create finding: '+e.message)}}
async function createCyberTicket(tool){const run=CYBER_LAST_RUN[tool];if(!run)return;try{const ticket=await json('/api/v1/cyber-tools/runs/'+run.id+'/create-ticket',{method:'POST'});alert((ticket.ticket_number||'Ticket')+' created.');await loadCyberHistory()}catch(e){alert('Could not create ticket: '+e.message)}}
async function scheduleCyberTool(tool){try{const payload=await cyberToolPayload(tool),value=prompt('Run every how many minutes? Minimum 15.','1440');if(value===null)return;const interval=Number(value);if(!Number.isFinite(interval)||interval<15){alert('Enter 15 minutes or more.');return}payload.interval_minutes=Math.round(interval);const row=await json('/api/v1/cyber-tools/schedules',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});alert('Schedule created. Next run: '+new Date(row.next_run_at).toLocaleString());await loadCyberSchedules()}catch(e){alert('Could not create schedule: '+e.message)}}
async function deleteCyberSchedule(id){if(!confirm('Delete this recurring Cyber Tool schedule?'))return;try{await json('/api/v1/cyber-tools/schedules/'+id,{method:'DELETE'});await loadCyberSchedules()}catch(e){alert('Could not delete schedule: '+e.message)}}
async function loadCyberCapabilities(){try{const caps=await json('/api/v1/cyber-tools/capabilities');Object.entries(caps).forEach(([tool,cap])=>{const badge=document.getElementById('cyber-cap-'+tool),button=document.querySelector(`[data-cyber-run="${tool}"]`);if(badge){badge.textContent=cap.available?(cap.engine||'Available'):'Setup required';badge.title=cap.source||cap.engine||'';badge.className='cyber-tool-status '+(cap.available?'ready':'unavailable')}if(button)button.disabled=!cap.available;if(tool==='evidence_capture'){const select=document.getElementById('cyber-evidence-interface'),current=select?.value;if(select){const interfaces=cap.interfaces||[];select.innerHTML=interfaces.length?interfaces.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join(''):'<option value="">No interfaces found</option>';if(current&&interfaces.includes(current))select.value=current}}})}catch(e){document.querySelectorAll('[id^="cyber-cap-"]').forEach(x=>{x.textContent='Unavailable';x.className='cyber-tool-status unavailable'})}}
async function loadCyberHistory(){const el=document.getElementById('cyberHistory');if(!el)return;try{const rows=await json('/api/v1/cyber-tools/runs?limit=30');el.innerHTML=rows.length?rows.map(run=>`<div class="cyber-history-row"><div><div class="cyber-history-tool">${esc(CYBER_TOOL_NAMES[run.tool]||run.tool)}</div><div class="cyber-history-meta">${new Date(run.completed_at).toLocaleString()}</div></div><div><b>${esc(run.summary||'Completed')}</b><div class="cyber-history-meta">${esc(run.target||'GODSEYE appliance')} · ${esc(run.profile)}</div></div><div class="actions"><span class="cyber-history-state ${run.status==='failed'?'failed':''}">${esc(run.status)}</span><a class="secondary" href="/api/v1/cyber-tools/runs/${run.id}/export">Export</a></div></div>`).join(''):'<div class="empty">No Cyber Tool runs yet.</div>'}catch(e){el.innerHTML='<div class="empty">Could not load scan history: '+esc(e.message)+'</div>'}}
async function loadCyberSchedules(){const el=document.getElementById('cyberSchedules');if(!el)return;try{const rows=await json('/api/v1/cyber-tools/schedules');el.innerHTML=rows.length?rows.map(row=>`<div class="cyber-schedule-row"><div><b>${esc(CYBER_TOOL_NAMES[row.tool]||row.tool)}</b><div class="cyber-history-meta">Every ${row.interval_minutes} min · next ${new Date(row.next_run_at).toLocaleString()}</div></div><button class="danger" onclick="deleteCyberSchedule(${row.id})">Delete</button></div>`).join(''):'<div class="empty">No recurring checks scheduled.</div>'}catch(e){el.innerHTML='<div class="empty">Could not load schedules: '+esc(e.message)+'</div>'}}
async function loadCyberTools(){await Promise.all([loadCyberCapabilities(),loadCyberHistory(),loadCyberSchedules()])}
const VIEW_LOADERS={
...
function v430CpuPercent(check){const text=String(check?.detail||'');const percent=text.match(/([0-9]+(?:\.[0-9]+)?)\s*%/);if(percent)return Number(percent[1]);const raw=Number(check?.value);if(!Number.isFinite(raw))return null;const cores=(typeof navigator!=='undefined'&&Number(navigator.hardwareConcurrency))||1;return Math.max(0,Math.min(100,raw/cores*100))}
function v430HealthDetail(check){const name=String(check?.name||'').toLowerCase(),detail=String(check?.detail||'');if(!name.includes('temperature'))return detail;return detail.replace(/(-?[0-9]+(?:\.[0-9]+)?)\s*°?c/ig,(_,value)=>((Number(value)*9/5)+32).toFixed(1)+' °F')}
function v430HealthMetrics(appliance){const checks=Array.isArray(appliance?.checks)?appliance.checks:[];const find=(words)=>checks.find(c=>words.some(w=>String(c.name||'').toLowerCase().includes(w)));const pct=(c)=>{const t=String(c?.detail||'');const m=t.match(/([0-9]+(?:\.[0-9]+)?)\s*%/);return m?+m[1]:null};const temp=(c)=>{const t=String(c?.detail||'');const m=t.match(/([0-9]+(?:\.[0-9]+)?)\s*°?c/i);return m?+m[1]:null};v430SetMeter('v430Cpu',v430CpuPercent(find(['cpu','load'])));v430SetMeter('v430Mem',pct(find(['memory','ram'])));v430SetMeter('v430Disk',pct(find(['disk','storage'])));const tv=temp(find(['temperature','thermal'])),te=document.getElementById('v430Temp'),tb=document.getElementById('v430TempBar');if(te)te.textContent=tv==null?'—':((tv*9/5)+32).toFixed(0)+'°F';if(tb)tb.style.width=Math.min(100,(tv||0)/85*100)+'%'}
function v430RenderServices(settings,checks){const el=document.getElementById('v430Services');const arr=Array.isArray(settings)?settings:[];const count=document.getElementById('dashboardServiceCount');if(count)count.textContent=arr.filter(x=>x.enabled!==false).length;if(!el)return;const recent=Array.isArray(checks)?checks:[];if(!arr.length){el.innerHTML='<div class="empty">No monitors configured.</div>';return}el.innerHTML=arr.slice(0,6).map(m=>{const hit=recent.find(c=>String(c.monitor_id||c.setting_id||'')===String(m.id||''))||recent.find(c=>!c.monitor_id&&!c.setting_id&&String(c.kind||'')===String(m.kind||'')&&String(c.target||'')===String(m.target||''));const state=String(hit?.status||'unknown').toLowerCase();const ok=hit?['up','ok','healthy','online','running','success'].includes(state):m.enabled!==false;return `<div class="v430-service-row"><span>${esc(m.name||m.target||m.check_type||'Monitor')}</span><span><i class="v430-live-dot" style="background:${ok?'#41dfa1':'#ff5968'}"></i>${ok?'Running':'Attention'}</span><span>${esc(m.check_type||'Check')}</span></div>`}).join('')}
...
async function startRemoteSession(agentId){
 try{
  const r=await remoteApi('/api/v1/remote-access/sessions',{method:'POST',body:JSON.stringify({agent_id:agentId})});REMOTE_SESSION=r.session;REMOTE_FRAME_SEQ=0;
...
async function pollRemoteSession(){
 ...
 try{
  ...
  if(s.status==='active'){
   empty.style.display='none';img.style.display='block';const frameWidth=Number(s.last_width||0),frameHeight=Number(s.last_height||0);if(frameWidth>0&&frameHeight>0){img.width=frameWidth;img.height=frameHeight;img.dataset.remoteWidth=String(frameWidth);img.dataset.remoteHeight=String(frameHeight);}const frameSeq=Number(s.frame_seq||0);if(frameSeq>0&&frameSeq!==REMOTE_FRAME_SEQ){REMOTE_FRAME_SEQ=frameSeq;img.src=`${s.frame_url}?t=${Date.now()}`;}document.getElementById('remoteScreenshotBtn').disabled=false;const cb=document.getElementById('remoteControlBtn');cb.disabled=!['view_only','denied'].includes(s.control_status);cb.textContent=s.control_status==='requested'?'Waiting for Control Approval…':s.control_status==='approved'?'Control Approved':'Request Control';
  }else if(pending){
...
class CyberToolRunRequest(BaseModel):
    tool: str
    target: str = ""
    profile: str = "standard"
    options: dict = {}

class CyberToolScheduleRequest(CyberToolRunRequest):
    interval_minutes: int = 1440

CYBER_TOOL_TITLES={"network_exposure":"Network exposure scan","endpoint_posture":"Endpoint security posture","web_tls":"Web and TLS audit","malware_ioc":"Malware and IOC scan","dns_email":"DNS and email security audit","linux_audit":"Linux security audit","threat_detection":"Network threat detection review","evidence_capture":"Network evidence capture"}

def _cyber_finding(c,row):
    if row["finding_id"]:
        found=c.execute("SELECT * FROM network_issues WHERE id=?",(row["finding_id"],)).fetchone()
        if found:return found
    try:result=json.loads(row["result_json"] or "{}")
    except Exception:result={}
    severe=(row["tool"]=="malware_ioc" and not result.get("ok")) or (row["tool"]=="threat_detection" and int(result.get("high_priority_count") or 0)>0)
    severity="critical" if severe else("high" if not result.get("ok") else"warning")
    recommendations={"network_exposure":"Confirm each exposed service is required, patched, and restricted by firewall rules.","endpoint_posture":"Bring offline agents online and review Windows Update and malware-scan results.","web_tls":"Enable HTTPS and missing browser security headers after compatibility testing.","malware_ioc":"Isolate the affected endpoint and follow the approved incident-response process.","dns_email":"Publish and validate missing DNS and email authentication controls.","linux_audit":"Review Lynis warnings and apply changes through normal change control.","threat_detection":"Validate the Suricata alert and affected assets before containment.","evidence_capture":"Preserve the PCAP with the case and review it using authorized analysis tools."}
    ts=now();title=f"{CYBER_TOOL_TITLES.get(row['tool'],'Cyber Tool result')}: {row['target'] or 'GODSEYE appliance'}"[:240];evidence=json.dumps({"cyber_run_id":row["id"],"tool":row["tool"],"summary":row["summary"],"result":result},default=str)[:12000]
    fid=c.execute("INSERT INTO network_issues(issue_type,severity,target,title,evidence,recommendation,status,first_seen,last_seen) VALUES('cyber_tool',?,?,?,?,?,'open',?,?)",(severity,row["target"][:255],title,evidence,recommendations.get(row["tool"],"Review and remediate through approved change control."),ts,ts)).lastrowid
    c.execute("UPDATE cyber_tool_runs SET finding_id=? WHERE id=?",(fid,row["id"]));return c.execute("SELECT * FROM network_issues WHERE id=?",(fid,)).fetchone()

@app.get(f"{router_prefix}/cyber-tools/capabilities")
def cyber_tool_capabilities(user=Depends(get_current_user)):return cyber_capabilities(DB_PATH.parent)

@app.post(f"{router_prefix}/cyber-tools/run")
def cyber_tool_run(req: CyberToolRunRequest,request: Request,user=Depends(require_permission("operate"))):
    try:
        with db() as c:
            ensure_cyber_schema(c);result=execute_cyber_tool(req.tool.strip(),req.target.strip(),req.profile.strip(),req.options or {},DB_PATH.parent,c);row=record_cyber_run(c,req.tool.strip(),req.target.strip(),req.profile.strip(),result,user["username"],cyber_utcnow());audit(c,user["username"],"cyber_tool_run",str(row["id"]),json.dumps({"tool":req.tool,"target":req.target,"ok":bool(result.get("ok"))}),client_ip(request));return row
    except ValueError as exc:raise HTTPException(400,str(exc))

@app.get(f"{router_prefix}/cyber-tools/runs")
def cyber_tool_runs(limit:int=50,tool:str="",user=Depends(get_current_user)):
    with db() as c:
        ensure_cyber_schema(c);rows=c.execute("SELECT * FROM cyber_tool_runs WHERE tool=? ORDER BY id DESC LIMIT ?",(tool,max(1,min(limit,200)))).fetchall() if tool else c.execute("SELECT * FROM cyber_tool_runs ORDER BY id DESC LIMIT ?",(max(1,min(limit,200)),)).fetchall()
    return[public_cyber_run(r) for r in rows]

@app.get(f"{router_prefix}/cyber-tools/runs/{{run_id}}/export")
def cyber_tool_export(run_id:int,user=Depends(get_current_user)):
    with db() as c:
        ensure_cyber_schema(c);row=c.execute("SELECT * FROM cyber_tool_runs WHERE id=?",(run_id,)).fetchone()
        if not row:raise HTTPException(404,"Cyber Tool run not found")
        payload=json.dumps(public_cyber_run(row),indent=2,default=str).encode()
    return Response(payload,media_type="application/json",headers={"Content-Disposition":f'attachment; filename="godseye-cyber-run-{run_id}.json"'})

@app.post(f"{router_prefix}/cyber-tools/runs/{{run_id}}/create-finding")
def cyber_tool_create_finding(run_id:int,request:Request,user=Depends(require_permission("operate"))):
    with db() as c:
        ensure_cyber_schema(c);row=c.execute("SELECT * FROM cyber_tool_runs WHERE id=?",(run_id,)).fetchone()
        if not row:raise HTTPException(404,"Cyber Tool run not found")
        found=_cyber_finding(c,row);audit(c,user["username"],"cyber_finding_created",str(found["id"]),json.dumps({"run_id":run_id}),client_ip(request));return dict(found)

@app.post(f"{router_prefix}/cyber-tools/runs/{{run_id}}/create-ticket")
def cyber_tool_create_ticket(run_id:int,request:Request,user=Depends(require_permission("operate"))):
    with db() as c:
        ensure_cyber_schema(c);row=c.execute("SELECT * FROM cyber_tool_runs WHERE id=?",(run_id,)).fetchone()
        if not row:raise HTTPException(404,"Cyber Tool run not found")
        if row["ticket_id"]:
            existing=c.execute("SELECT * FROM tickets WHERE id=?",(row["ticket_id"],)).fetchone()
            if existing:return dict(existing)
        found=_cyber_finding(c,row);priority="critical" if found["severity"]=="critical" else("high" if found["severity"]=="high" else"medium");ts=now();tid=c.execute("INSERT INTO tickets(title,description,status,priority,assignee,linked_type,linked_id,device_name,created_by,created_at,updated_at) VALUES(?,?,'open',?,'','network_finding',?,?,?, ?,?)",(found["title"],found["recommendation"],priority,found["id"],row["target"][:255],user["username"],ts,ts)).lastrowid;number=f"TKT-{tid:05d}";c.execute("UPDATE tickets SET ticket_number=? WHERE id=?",(number,tid));c.execute("UPDATE cyber_tool_runs SET ticket_id=? WHERE id=?",(tid,run_id));audit(c,user["username"],"ticket_created_from_cyber_tool",number,json.dumps({"run_id":run_id}),client_ip(request));return dict(c.execute("SELECT * FROM tickets WHERE id=?",(tid,)).fetchone())

@app.get(f"{router_prefix}/cyber-tools/schedules")
def cyber_tool_schedules(user=Depends(get_current_user)):
    with db() as c:ensure_cyber_schema(c);rows=c.execute("SELECT * FROM cyber_tool_schedules ORDER BY enabled DESC,next_run_at,id").fetchall()
    out=[]
    for r in rows:
        d=dict(r)
        try:d["options"]=json.loads(d.pop("options_json") or "{}")
        except Exception:d["options"]={}
        d["enabled"]=bool(d["enabled"]);out.append(d)
    return out

@app.post(f"{router_prefix}/cyber-tools/schedules")
def cyber_tool_schedule_create(req:CyberToolScheduleRequest,request:Request,user=Depends(require_permission("operate"))):
    tool=req.tool.strip();profile=req.profile.strip();interval=max(15,min(int(req.interval_minutes),43200))
    if tool not in SCHEDULABLE_TOOLS:raise HTTPException(400,"This Cyber Tool cannot be scheduled")
    if profile not in{"quick","standard","deep"}:raise HTTPException(400,"Invalid profile")
    ts=cyber_utcnow();nxt=(dt.datetime.now(dt.timezone.utc)+dt.timedelta(minutes=interval)).isoformat()
    with db() as c:ensure_cyber_schema(c);sid=c.execute("INSERT INTO cyber_tool_schedules(tool,target,profile,options_json,interval_minutes,enabled,created_by,created_at,updated_at,next_run_at) VALUES(?,?,?,?,?,1,?,?,?,?)",(tool,req.target.strip()[:2048],profile,json.dumps(req.options or {},separators=(',',':')),interval,user["username"],ts,ts,nxt)).lastrowid;audit(c,user["username"],"cyber_schedule_created",str(sid),json.dumps({"tool":tool,"interval":interval}),client_ip(request));return dict(c.execute("SELECT * FROM cyber_tool_schedules WHERE id=?",(sid,)).fetchone())

@app.delete(f"{router_prefix}/cyber-tools/schedules/{{schedule_id}}")
def cyber_tool_schedule_delete(schedule_id:int,request:Request,user=Depends(require_permission("operate"))):
    with db() as c:
        ensure_cyber_schema(c);row=c.execute("SELECT * FROM cyber_tool_schedules WHERE id=?",(schedule_id,)).fetchone()
        if not row:raise HTTPException(404,"Schedule not found")
        c.execute("DELETE FROM cyber_tool_schedules WHERE id=?",(schedule_id,));audit(c,user["username"],"cyber_schedule_deleted",str(schedule_id),row["tool"],client_ip(request))
    return{"ok":True}

@app.get(f"{router_prefix}/cyber-tools/evidence/{{filename}}")
def cyber_tool_evidence(filename:str,user=Depends(get_current_user)):
    if not re.fullmatch(r"capture-[A-Za-z0-9TZ-]+\.pcap",filename):raise HTTPException(404,"Evidence file not found")
    root=(DB_PATH.parent/"cyber-evidence").resolve();path=(root/filename).resolve()
    if path.parent!=root or not path.is_file():raise HTTPException(404,"Evidence file not found")
    return FileResponse(path,media_type="application/vnd.tcpdump.pcap",filename=filename,headers={"Cache-Control":"no-store"})
