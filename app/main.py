...
VALID_DEVICE_ICONS = {"auto", "router", "switch", "access-point", "pc", "laptop", "server", "nas", "camera", "printer", "phone", "tablet", "tv", "game-console", "iot", "other"}
...
app = FastAPI(title="GODSEYE", version="2.4.0", lifespan=lifespan)

@app.get("/assets/login-bg.jpg", include_in_schema=False)
def login_background():
    return FileResponse(BASE_DIR / "app" / "assets" / "login-bg.jpg", media_type="image/jpeg", headers={"Cache-Control":"public, max-age=86400"})
...
.device-icon{width:32px;height:32px;display:inline-flex;align-items:center;justify-content:center;flex:0 0 32px}.device-icon img{width:30px;height:30px;object-fit:contain;display:block}.device-icon-picker{display:grid;grid-template-columns:repeat(5,minmax(72px,1fr));gap:10px}.device-icon-choice{border:1px solid #d8e3ef;background:#fff;border-radius:8px;padding:9px 6px;cursor:pointer;text-align:center;color:#40536b;font-size:11px}.device-icon-choice:hover,.device-icon-choice.selected{border-color:#0d74f5;box-shadow:0 0 0 2px rgba(13,116,245,.10)}.device-icon-choice img{width:38px;height:38px;display:block;object-fit:contain;margin:0 auto 5px}.custom-icon-preview{width:48px;height:48px;object-fit:contain;border:1px solid #d8e3ef;border-radius:8px;background:#fff;padding:4px}.icon-upload-row{display:flex;align-items:center;gap:12px;flex-wrap:wrap}@media(max-width:700px){.device-icon-picker{grid-template-columns:repeat(3,1fr)}}
...
<div id="authOverlay" class="overlay" style="display:none">
  ...
  <div class="login-scene-footer">GODSEYE v2.3.0<br>Network Intelligence · Device Correlation · Smart Alerts</div>
</div>
...
<div class="hero"><div><h1>Network Map</h1><div class="muted">Interactive topology view of your gateway, infrastructure, and connected clients</div></div><div class="actions"><button class="primary" onclick="runFullDiscovery()">⟳ Full Discovery</button><button class="secondary" onclick="loadNetwork()">↻ Refresh</button></div></div>
<div class="map-summary"><div class="map-summary-card"><div class="map-summary-label">Gateway</div><div class="map-summary-value" id="mapGateway">—</div></div><div class="map-summary-card"><div class="map-summary-label">Discovered Nodes</div><div class="map-summary-value" id="mapNodes">—</div></div><div class="map-summary-card"><div class="map-summary-label">Relationships</div><div class="map-summary-value" id="mapLinks">—</div></div><div class="map-summary-card"><div class="map-summary-label">Last Updated</div><div class="map-summary-value" id="mapUpdated">—</div></div></div>
<div class="map-shell">
<div class="map-main"><section class="panel map-card"><div class="table-head"><h2>Live Network Topology</h2><div class="muted" id="mapStatus">Loading topology…</div></div><div class="map-toolbar2"><input id="mapSearch" class="input" placeholder="Find a device, IP, MAC or vendor…" oninput="filterMapNodes()"><div class="map-zoom"><button type="button" title="Zoom out" onclick="zoomNetwork(-0.1)">−</button><button type="button" title="Fit map" onclick="fitNetworkMap()">Fit</button><button type="button" title="Zoom in" onclick="zoomNetwork(0.1)">＋</button></div></div><div class="map-viewport" id="networkViewport"><div class="map-stage" id="networkStage"><div class="map-canvas" id="networkCanvas"><div class="map-empty">Building your network map…</div></div></div></div></section></div>
<aside class="map-side"><div class="map-side-card"><h3>Map Legend</h3><div class="map-legend-row"><span class="map-legend-dot"></span> Router / network node</div><div class="map-legend-row"><span class="map-legend-dot green"></span> Online client</div><div class="map-legend-row"><span class="map-legend-dot gray"></span> Offline client</div><div class="map-legend-row"><span class="map-legend-dot purple"></span> Infrastructure</div><div class="map-legend-row"><span class="map-legend-dot red"></span> Client needing attention</div></div><div class="map-side-card"><h3>How to use the map</h3><div class="map-tip">Click a discovered client to open its device details. Use the search box to highlight a device, the zoom controls to adjust the topology, and Full Discovery to refresh relationship evidence from the network.</div></div><div class="map-side-card"><h3>Topology Evidence</h3><div class="map-tip" id="mapEvidence">Relationship labels show whether a path came from UniFi, SNMP, local neighbor data, or a conservative gateway inference.</div></div></aside>
</div>
</div>
...
<div id="deviceIconModal" class="modal" style="display:none">
  <div class="modal-card" style="max-width:650px">
    <div class="modal-head"><div><h2>Select Device Icon</h2><div class="muted">Choose an icon that makes the device easy to recognize in GODSEYE.</div></div><button class="icon-btn" onclick="closeDeviceIcon()">×</button></div>
...
const DEVICE_ICON_LABELS={auto:'Auto',router:'Router',switch:'Switch','access-point':'Access Point',pc:'PC',laptop:'Laptop',server:'Server',nas:'NAS',camera:'Camera',printer:'Printer',phone:'Phone',tablet:'Tablet',tv:'TV','game-console':'Game Console',iot:'IoT',other:'Other'};
const DEVICE_ICON_KEYS=Object.keys(DEVICE_ICON_LABELS);
function inferredDeviceIcon(type){const t=String(type||'').toLowerCase();if(t.includes('router')||t.includes('gateway'))return'router';if(t.includes('switch'))return'switch';if(t.includes('access point')||t==='ap'||t.includes('wifi'))return'access-point';if(t.includes('laptop')||t.includes('notebook'))return'laptop';if(t==='pc'||t.includes('desktop')||t.includes('computer')||t.includes('workstation'))return'pc';if(t.includes('camera')||t.includes('nvr'))return'camera';if(t.includes('printer'))return'printer';if(t.includes('phone')||t.includes('iphone')||t.includes('android'))return'phone';if(t.includes('tablet')||t.includes('ipad'))return'tablet';if(t.includes('server'))return'server';if(t.includes('nas')||t.includes('storage'))return'nas';if(t.includes('tv')||t.includes('media')||t.includes('stream'))return'tv';if(t.includes('game')||t.includes('console')||t.includes('xbox')||t.includes('playstation'))return'game-console';if(t.includes('iot')||t.includes('smart')||t.includes('sensor')||t.includes('light'))return'iot';return'other'}
function effectiveDeviceIcon(d){return(d&&d.icon_key&&d.icon_key!=='auto')?d.icon_key:inferredDeviceIcon(d&&d.device_type)}
function deviceIconSrc(d){if(d&&d.icon_data)return d.icon_data;return '/assets/device-icons/'+effectiveDeviceIcon(d)+'.svg'}
...
function renderDeviceIconPicker(selected){const p=document.getElementById('deviceIconPicker');if(!p)return;p.innerHTML=DEVICE_ICON_KEYS.map(k=>`<button type="button" class="device-icon-choice ${k===selected?'selected':''}" data-icon-key="${k}" onclick="selectDeviceIcon('${k}')"><img src="/assets/device-icons/${k==='auto'?'other':k}.svg" alt=""><span>${esc(DEVICE_ICON_LABELS[k])}</span></button>`).join('')}
function selectDeviceIcon(key){deviceIconKey.value=key;deviceIconData.value='';deviceIconPreview.style.display='none';deviceIconFile.value='';renderDeviceIconPicker(key)}
function openDeviceIconFromButton(btn){deviceIconId.value=btn.dataset.iconId;deviceIconKey.value=btn.dataset.iconKey||'auto';deviceIconData.value='';deviceIconIdentity.textContent=btn.dataset.iconIdentity||'';deviceIconErr.textContent='';deviceIconFile.value='';deviceIconPreview.style.display='none';renderDeviceIconPicker(deviceIconKey.value);deviceIconModal.style.display='grid'}
function closeDeviceIcon(){deviceIconModal.style.display='none'}
function previewCustomDeviceIcon(input){const f=input.files&&input.files[0];deviceIconErr.textContent='';if(!f)return;if(!['image/png','image/jpeg','image/webp'].includes(f.type)){deviceIconErr.textContent='Choose a PNG, JPEG, or WebP image.';input.value='';return}if(f.size>262144){deviceIconErr.textContent='Custom icon must be 256 KB or smaller.';input.value='';return}const r=new FileReader();r.onload=()=>{deviceIconData.value=String(r.result||'');deviceIconKey.value='other';deviceIconPreview.src=deviceIconData.value;deviceIconPreview.style.display='block';renderDeviceIconPicker('__custom__')};r.onerror=()=>{deviceIconErr.textContent='Could not read the image.'};r.readAsDataURL(f)}
...
async function loadInventory(){
  ...
  try{const q=new URLSearchParams();if(se?.value)q.set('search',se.value);if(st?.value)q.set('status',st.value);const response=await json('/api/v1/devices?'+q.toString()),d=Array.isArray(response)?response:[];if(ce)ce.textContent=d.length+' devices';tbody.innerHTML=d.length?d.map(x=>`<tr onclick="goDevice(${x.id})" style="cursor:pointer"><td><div class="device-name">${deviceIconHtml(x)}<div><button class="link name" onclick="event.stopPropagation();goDevice(${x.id})">${esc(x.name||x.hostname||'Unknown device')}</button><div class="muted">${esc(x.device_type||'Unclassified')}</div></div></div></td><td>${esc(x.ip||'—')}</td><td>${esc(x.mac||'—')}</td><td>${esc(x.vendor||'—')}</td><td>${esc(x.device_type||'—')}</td><td><span class="status-badge">${esc(CLASS_LABEL[x.classification]||x.classification||'new')}</span></td><td>${x.last_seen?esc(new Date(x.last_seen).toLocaleString()):'—'}</td><td><span class="status-badge">${esc(x.status||'unknown')}</span></td><td><button class="secondary" data-rename-id="${x.id}" data-rename-name="${esc(x.name||'')}" data-rename-identity="${esc([x.hostname,x.ip,x.mac].filter(Boolean).join(' · '))}" onclick="event.stopPropagation();openRenameDeviceFromButton(this)">${x.name?'Rename':'Name'}</button> <button class="secondary" data-classify-id="${x.id}" data-classify-type="${esc(x.device_type||'')}" data-classify-class="${esc(x.classification||'new')}" data-classify-identity="${esc([x.name||x.hostname,x.ip,x.mac].filter(Boolean).join(' · '))}" onclick="event.stopPropagation();openClassifyDeviceFromButton(this)">Classify</button> <button class="secondary" data-icon-id="${x.id}" data-icon-key="${esc(x.icon_key||'auto')}" data-icon-identity="${esc([x.name||x.hostname,x.ip,x.mac].filter(Boolean).join(' · '))}" onclick="event.stopPropagation();openDeviceIconFromButton(this)">Icon</button> <button class="secondary" onclick="event.stopPropagation();goDevice(${x.id})">Details</button></td></tr>`).join(''):'<tr><td colspan="9" class="empty">No devices discovered yet. Click Scan Now to discover the network.</td></tr>'}catch(e){tbody.innerHTML='<tr><td colspan="9" class="empty">Unable to load devices: '+esc(e.message)+'</td></tr>'}
}


function mapNodeClass(n){const status=String(n.status||'').toLowerCase();const type=String(n.type||'').toLowerCase();if(status.includes('offline'))return 'offline-node';if(status.includes('investigate')||status.includes('issue'))return 'issue-node';if(type.includes('switch')||type.includes('access')||type.includes('infrastructure'))return 'infrastructure-node';return ''}
function mapIcon(n){const type=String(n.type||'').toLowerCase();if(type==='internet')return '☁';if(type==='gateway')return '▣';if(type.includes('switch'))return '▦';if(type.includes('wifi')||type.includes('access'))return '◉';if(type.includes('phone')||type.includes('mobile'))return '▯';if(type.includes('camera'))return '◉';if(type.includes('printer'))return '▤';return '●'}
function filterMapNodes(){const q=(document.getElementById('mapSearch')?.value||'').trim().toLowerCase();document.querySelectorAll('#networkCanvas .map-node[data-search]').forEach(n=>{const match=!q||n.dataset.search.includes(q);n.style.opacity=match?'1':'.2';n.style.filter=match?'none':'grayscale(.7)';if(q&&match)n.style.borderColor='#0d74f5';else n.style.borderColor=''})}
...
function zoomNetwork(delta){NETWORK_ZOOM=Math.max(.6,Math.min(1.5,Math.round((NETWORK_ZOOM+delta)*10)/10));applyNetworkZoom()}
function fitNetworkMap(){const vp=document.getElementById('networkViewport'),stage=document.getElementById('networkStage');if(!vp||!stage)return;NETWORK_ZOOM=Math.max(.6,Math.min(1,(vp.clientWidth-40)/Math.max(760,stage.scrollWidth)));applyNetworkZoom();vp.scrollTo({left:0,top:0,behavior:'smooth'})}
window.filterMapNodes=filterMapNodes;window.zoomNetwork=zoomNetwork;window.fitNetworkMap=fitNetworkMap;
async function loadNetwork(){
  const canvas=document.getElementById('networkCanvas');if(!canvas)return;
  const status=document.getElementById('mapStatus');if(status)status.textContent='Refreshing topology…';
  try{
    const t=await json('/api/v1/intelligence/topology/live'),nodes=t.nodes||[],links=t.links||[],byId={};nodes.forEach(n=>byId[n.id]=n);
    const gateway=t.gateway||'gateway',childLinks={};links.forEach(l=>{(childLinks[l.parent]||(childLinks[l.parent]=[])).push(l)});
    const gw=document.getElementById('mapGateway'),nc=document.getElementById('mapNodes'),lc=document.getElementById('mapLinks'),up=document.getElementById('mapUpdated');
    if(gw)gw.textContent=t.gateway||'Unknown';if(nc)nc.textContent=nodes.filter(n=>n.type!=='internet').length;if(lc)lc.textContent=links.length;if(up)up.textContent=new Date(t.generated_at||Date.now()).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'});
    const card=(n,link)=>{const searchable=[n.label,n.ip,n.mac,n.vendor,n.type,n.status].filter(Boolean).join(' ').toLowerCase();const clickable=n.device_id?` device-clickable" onclick="goDevice(${Number(n.device_id)})`:'"';const evidence=link?.details?.source?` · ${esc(link.details.source)}`:'';return `<div class="map-node ${mapNodeClass(n)}${clickable} data-search="${esc(searchable)}" title="${n.device_id?'Open device details':esc(n.type||'Network node')}"><div class="map-circle">${mapIcon(n)}</div><div class="map-label">${esc(n.label||n.id)}</div><div class="map-sub">${esc(n.ip||n.mac||n.type||'—')}</div><div class="map-sub">${esc(n.vendor||'')}${n.status?' · '+esc(String(n.status).replaceAll('_',' ')):''}</div>${link?`<span class="map-link-badge">${esc(link.link_type||'link')}${evidence}</span>`:''}</div>`};
    const renderBranch=(link,depth=0)=>{const n=byId[link.child];if(!n)return'';const kids=(childLinks[n.id]||[]).filter(x=>x.child!==gateway).slice(0,16);return `<div style="display:flex;flex-direction:column;align-items:center;gap:6px">${card(n,link)}${kids.length&&depth<2?`<div class="map-line" style="height:18px"></div><div class="map-children">${kids.map(k=>renderBranch(k,depth+1)).join('')}</div>`:''}</div>`};
    const roots=(childLinks[gateway]||[]).filter(l=>byId[l.child]),attached=new Set();links.forEach(l=>attached.add(l.child));
    const extra=nodes.filter(n=>!['internet',gateway].includes(n.id)&&!attached.has(n.id)).map(n=>({parent:gateway,child:n.id,link_type:'gateway_path',details:{source:'inventory'}}));
    const branches=[...roots,...extra].slice(0,60).map(l=>renderBranch(l)).join('');
    canvas.innerHTML=`${card(byId.internet||{id:'internet',label:'Internet',type:'internet',status:'online'})}<div class="map-line"></div>${card(byId[gateway]||{id:gateway,label:'Gateway / Router',type:'gateway',ip:t.gateway,status:'online'},(childLinks.internet||[])[0])}<div class="map-line"></div><div class="map-children">${branches||'<div class="map-node"><div class="map-circle">?</div><div class="map-label">No clients yet</div><div class="map-sub">Run Full Discovery</div></div>'}</div>`;
    if(status)status.textContent=nodes.length+' nodes · '+links.length+' relationships';
    const evidence=document.getElementById('mapEvidence');if(evidence){const sources=[...new Set(links.map(l=>l.details?.source).filter(Boolean))];evidence.textContent=sources.length?'Evidence sources: '+sources.join(', ')+'. Relationship badges identify how each path was learned.':'Relationship badges identify discovered or inferred network paths.'}
    filterMapNodes();setTimeout(fitNetworkMap,20);
  }catch(e){canvas.innerHTML='<div class="map-empty">Topology is not available yet.<br><span class="muted">'+esc(e.message)+'</span><br><br><button class="primary" onclick="runFullDiscovery()">Run Full Discovery</button></div>';if(status)status.textContent='Topology unavailable'}
}
