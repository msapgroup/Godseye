...
def pihole_analytics(base_url, token=None):
    headers = {'User-Agent':'GODSEYE/1.4'}
    if token:
    ...
    for path in candidates:
        try:
            data=_http_json(base_url.rstrip('/')+path, headers=headers)
            if isinstance(data, dict):
                ...
                clients = data.get('clients') or data.get('unique_clients') or data.get('unique_clients')
                return {'ok':True,'queries':queries,'blocked':blocked,'clients':clients,'raw':data,'endpoint':path}
...
def sync_integration(c, kind: str):
    ...
    try:
        ...
        if kind=='pihole':
            result=pihole_client_observations(cfg['target'], cfg.get('secret') or None)
            observations=result.get('observations',[])
            count=ingest_observations(c,'pihole',observations)
            analytics=pihole_analytics(cfg['target'],cfg.get('secret') or None)
        elif kind=='unifi':
