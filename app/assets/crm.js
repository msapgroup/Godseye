...
function crmRenderCustomer(customer){
 ...
 <div id="crmContactEditor"></div>${existing?kbFiles(customer.files||[],'customer',customer.id):'<div class="crm-empty">Save the customer before uploading files.</div>'}${customer.site_id?`<div class="crm-linked-site">Linked site: <b>${crmEscape(customer.site_name||'Site unavailable')}</b>${customer.site_status?' · '+crmEscape(customer.site_status):''} <button class="secondary" type="button" onclick="showView('sites',true)">Open Sites</button></div>`:''}`;
