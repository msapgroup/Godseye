...
if [[ -n "$PYTHON_BIN" ]]; then "$PYTHON_BIN" -c 'import sys; print("Python:", sys.version.split()[0]); raise SystemExit(0 if sys.version_info >= (3,9) else 1)' || echo "ERROR: Python 3.9+ is required."; fi
