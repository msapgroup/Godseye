...
The **GODSEYE Windows Agent** is the recommended Windows Event Findings collection method. Agent v1.1 is designed for 64-bit Windows workstations and Windows Server and adds operator-triggered **Pull Events Now** support.
...
Agent v1.1 checks GODSEYE every 10 seconds for defined management commands. Selecting **Pull Events Now** for one agent, or **Pull All Online** for all compatible online agents, queues a `pull_events` command. The Windows service receives the command over its existing outbound HTTPS connection, flushes any durable pending batch, performs a fresh bookmark-based collection, uploads new Critical/Error/Warning events, and reports the event/new-finding counts back to GODSEYE.

The 10-second management heartbeat is separate from the normal Event Log collection interval, so the configured 30–3600 second background schedule remains intact. GODSEYE coalesces duplicate pending pull requests so repeated clicks do not create a command storm.

Agent v1.0 continues scheduled collection, but **Pull Events Now** requires Agent v1.1.0 or newer.
...
## Upgrade an existing enrolled agent to v1.1

Copy the current agent package to the Windows endpoint, open 64-bit PowerShell as Administrator, and run:
...
The installer stops the service, recompiles the v1.1 executable, preserves the existing agent UUID/configuration and DPAPI-protected API key, then restarts the service. A new enrollment token is not required. If the endpoint still uses temporary `-SkipTlsVerify`, include that switch again until Windows trusts the GODSEYE HTTPS certificate.
...
- The service reads Event Logs and performs only defined GODSEYE operations such as event pulls and targeted Event Finding rechecks. It does **not** expose arbitrary command, PowerShell, or shell execution.
